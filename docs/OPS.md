...
- 📌 **THREE PRE-EXISTING BRANDING GAPS FOUND (none caused by this sync; fix separately):** (1) `frontend/src/core/agents/api.ts:101` + `:113` — user-facing error **"Could not reach the DeerFlow backend."**; (2) `frontend/src/core/sidecar/context.ts:162` — sidecar system prompt says "the user's current DeerFlow chat"; (3) `en-US.ts:573` / `zh-CN.ts:550` — **"WRI AI AI bot"** double-AI from a `DeerFlow`→`WRI AI` substitution. (1) and (2) are **hardcoded TS strings outside the locale files**, which is exactly why the i18n rebrand never caught them. Deliberate retentions that must STAY: `officialWebsite`, `githubTooltip`, `visitGithub`, and the Settings→About MIT credit — those correctly point at bytedance/deer-flow.
