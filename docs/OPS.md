...
Most recent upstream sync: **2026-07-29 (later)** — **6 commits (`4e449385`→`4582a413`), CLEAN merge (0 conflicts), MIGRATION-FREE (head stays `0010`), 24 files / +1261−175.** Merge `9b762282`. Verified inline. **`client.py` untouched (0 matches in range); root `.dockerignore` untouched** — ⚠ the carried-patch grep flagged a `dockerignore` hit but it was the NEW `docker/lark-cli-broker/Dockerfile.dockerignore` for an image we never build (check WHICH file the pattern matched, not just that it matched). `merge-tree` exit 0.
