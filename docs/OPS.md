...
Most recent upstream sync: **2026-07-20 (later²)** — **16 commits (`09e25b8a`→`b565e6c0`, i.e. everything after `1a1c5def`), CLEAN merge (0 conflicts), MIGRATION-FREE (head stays `0005`), NO CVE.** Merge `54c9a2b6`; `main` ff'd → `b565e6c0`. Scope: 43 files, +7563/−143 (mostly tests). **A bigger, feature-heavy batch — every new-behavior feature is INERT-BY-DEFAULT and verified from the diffs.** Highlights:
