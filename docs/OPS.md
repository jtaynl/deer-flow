...
- **Edge basic-auth + app auth (2026-08-30):** since #5041 the gateway 401s any request whose `Authorization` header is not a valid Bearer PAT — including the browser's replayed `Basic` credential from Caddy's `basic_auth`. Caddy MUST strip it: `header_up -Authorization` inside `reverse_proxy`. Symptom: login loop to `/login?next=…`, nginx log shows the basic-auth user on every request, gateway log shows `/api/features` 401.
...
2. **`PORT=2026` in `.env` + `BIND_HOST` UNSET** pins the compose `nginx` to loopback.
   ⚠ **Changed 2026-08-01 by upstream `#4618`.** The compose line is now
   `"${BIND_HOST:-127.0.0.1}:${PORT:-2026}:2026"`, i.e. upstream does natively what our old
   `PORT=127.0.0.1:2026` trick did. **Keeping the trick would expand to a malformed
   `127.0.0.1:127.0.0.1:2026:2026` and fail `make up`.** Leave `BIND_HOST` unset;
   set it to `0.0.0.0` ONLY behind a TLS/auth front door.
   *Historical note:* the old trick worked because the spec was plain `${PORT:-2026}:2026`, so
   `PORT=127.0.0.1:2026` expanded to `127.0.0.1:2026:2026`. That is no longer valid.
...
- ⚠⚠ **NEW 3c BROWSER ASSERTION CAUGHT A LATENT BREAK: the playwright MCP's browser was unusable** — every browser call failed `Browser "chrome-for-testing" is not installed` while `chromium-1234` sat unused. **Mechanism: `@playwright/mcp@0.0.75` bundles its OWN playwright (1.61.0-alpha) whose browser registry ≠ the build installed by the UNPINNED stable `playwright` in the Dockerfile.** Unknown how long latent — no prior smoke exercised a browser tool (the #4512 lesson generalises: probe every tool CLASS, not just the ones the pipeline uses). **Fixed durably in `backend/Dockerfile`** (install via the MCP's own `install-browser chrome-for-testing` entry point so the registry always matches) + rebuilt. **Proven at the layer that changed**: direct launch with the bundled playwright → `LAUNCH_OK title=WRI AI — Intelligence Assistant by World Research Institute`. Agent-driven browser runs remain model-dependent (qwen sometimes prefers web_fetch; one forced deepseek run looped) — the infra layer is certified, the web-UI Browser panel deserves an owner eyeball.
- **`#4758` subagent isolation verified live**: `SUBAGENT_OK 221` on a real delegated run — registry keys changed internally, model-visible contract byte-identical (it *closes* a cross-run collision). `#4657` also ends a false "Artifact delivery incomplete" class under always-on `#4494`, and its **60s MCP bring-up bound now applies to our live playwright server** (boot log clean, no skip lines). Todo-middleware fix is plan-mode-only (not our chain). Honcho/OpenViking purely additive (DeerMem untouched; new pinned dep `langchain-openviking` rides the image). Channels/TUI/dev commits dead code.
- **Verified (all green, re-certified on the FINAL containers after the browser-fix rebuild):** 3a boot clean, 0 tracebacks, migration applied → **3b** `schema in sync`, head `0011_mcp_tasks`, `mcp_tasks` table present (0 rows), all invariants unchanged (7 models, `qwen3.8-max` present, `mcp_tasks.enabled False`) → **3c** thread-id accept/reject, `chat()` ×3, UAE e2e, sandbox bash, present_files, **subagent smoke (new)**, **browser-launch probe (new)** → landing 12 WRI markers / 0 DeerFlow, `/showcase/not-a-demo` 404, **2951 `WRI AI` in bundle**, `/api/models` 401, `/docs` 404, loopback bind, public 401.
- **Carried patches:** i18n **32 en / 31 zh** (tally UPDATED this sync), nav 0, hero 0 Galaxy, deleted sections absent, redis-strip intact, `.env` intact, all 3 instance `config.yaml` items, **Dockerfile playwright patch EVOLVED** (browser-channel fix is now part of the carried patch). Backup: `~/deerflow-syncbak-20260812-063459/` (incl. `.env` — ⚠ plain `ls` hides dotfiles; verify backups with `ls -la`).

Earlier upstream sync: **2026-08-05** — **12 commits (`bf2cb19c`→`480a3757`), 1 conflict, MIGRATION-FREE (head stays `0010`), 92 files.** Merge `6834965a`. Reviewed by a 12-agent workflow (7 areas, adversarial verify on every HIGH/MEDIUM; 2 findings REFUTED into corrected forms, both mattered).

- **The conflict:** `case-study-section.tsx` modify/delete — `#4635` modified it, our re-skin deleted it. Keep-OURS (`git rm`); safe because `git grep` finds **zero** `CaseStudySection` references anywhere on `local-fixes`. The recurring `landing/` conflict surface again.
- 📌 **The review's real catch was PRE-EXISTING, not in this range: our tree had carried two DANGLING TESTS of the upstream landing since the `#4622` merge (2026-08-02)** — `tests/unit/components/landing/case-study-section.dom.test.tsx` (imports the deleted section → `pnpm test`/`check`/`tsc` fail) and `tests/unit/components/ui/animation-scheduling.test.ts` (readFileSync-asserts Galaxy/whats-new/skills patterns on files we re-skinned/deleted — all 3 cases fail by design). **Three syncs built green over them because the Docker build only runs `pnpm build`** — deploy-path safety held empirically, but the module test suites were broken. Both removed in the merge commit. ⚠ **LESSON: when deleting an upstream file, sweep for its TEST files too — including path-string consumers (`readFileSync`), which a module-import grep misses.** (A first-pass agent claimed "sole importer is page.tsx" off a grep scoped to `frontend/src`; the verify pass caught it by unscoping the grep.)
- **`#4636` middleware plugin foundation** — gated OFF: `AppConfig.plugins` defaults `[]`, loading only inside gateway `create_app()`; the embedded client sits on `EMPTY_EXTENSIONS`. **Verified by instantiation, not config dump** (per the `#4638` lesson): `get_loaded_extensions()` → `LoadedExtensions`, `has_middleware_contributors False`, `needs_task_store False`, `plugins: []`. Note: both middleware builders now route through `compose_with_extensions` unconditionally — same semantics, but new code on every `chat()`; covered by the 3c smokes. `plugins` is STARTUP_ONLY (restart, not reload). New `deerflow-extension-api` workspace package is a hard import from `deerflow.config` — **in-image verified** (`API_VERSION 0.1.0`); ⚠ the HOST `backend/.venv` lacks it, so host-side `.venv/bin/python` probes importing `deerflow.config` will fail until `uv sync` — all our routine probes are `docker exec`, so inert.
- **`#4677` presented-files runtime user** — the always-on `#4494` path. Embedded-path resolution provably lands on `'default'` (every branch falls through to `get_effective_user_id`). **Smoked end-to-end**: a run wrote `/mnt/user-data/outputs/pf_probe.txt`, called `present_files`, run completed, artifact verified **on disk in the default bucket**.
- **`#4668` sanitize**: str fast-path unchanged; LGI sends plain strings; channels (the emitters of list-form bare strings) disabled. **`#4615` recursion-limit**: CLI-only, embedded kwarg plumbing untouched — LGI's `recursion_limit=250/500` unaffected. **`#4649` Buzz/Nostr**: lazy registry; `coincurve` is an unused optional extra (`UV_EXTRAS=postgres` unchanged); config default `enabled:false` matches example (no silent drift; `config_version` 32→33, warn-only as always). **`27648137`**: dev-script only, no compose/Makefile/deploy.sh collision.
- **cryptography 49→50 (major)**: imported at gateway boot via `channel_connections` (Fernet) even with channels off. Boot clean + explicit probe: **`cryptography 50.0.0` Fernet roundtrip OK**.
- **New public `/showcase/[thread_id]` routes**: static demo data only (13 allowlisted DeerFlow-branded ids, no backend calls, `dynamicParams=false` — unknown id → **404 verified**). Orphaned in our tree (nothing links to them) and behind the Caddy 401 front door; kept to minimize divergence. Delete `frontend/src/app/showcase/` later only if DeerFlow-branded content on the WRI host becomes unacceptable.
- **Verified (all green):** 3 containers, 0 tracebacks, startup complete → **3b** `schema in sync`, head `0010`, all invariants unchanged (consolidation/injection/authz/scheduler off, `retrieval_adapter ''`, sandbox `1.11.0`, `checkpoint_channel_mode full`, 7 models, `qwen3.8-max` present, `plugins []`) → **3c** thread-id accept(64)+reject(65), `chat()` ×3 `CHAT_OK`, UAE e2e `UAE_THREAD_OK`, sandbox bash `COUNT=3`+`2:beta`, **present_files probe OK (new)** → landing 12 WRI markers / 0 DeerFlow / 0 Galaxy, `/login`+`/setup` WRI-branded, 2931 `WRI AI` in bundle, `/showcase/not-a-demo` 404, `/api/models` 401, `/docs` 404, loopback bind, public 401.
- **Carried patches:** i18n 30/29, nav 0, hero 0 Galaxy, deleted sections absent, redis-strip (compose + deploy.sh) intact, `.env` intact, all 3 instance `config.yaml` items present. Backup: `~/deerflow-syncbak-20260805-010133/`.

Earlier upstream sync: **2026-08-03** — **7 commits (`c0f1cfef`→`bf2cb19c`), CLEAN merge (0 conflicts), MIGRATION-FREE (head stays `0010`), 36 files.** Merge `b46bac1d`.

- **`#4638` `feat(checkpoint): checkpoint history cache` was the one commit worth real scrutiny** — it lands on our persistence path, ships a `checkpoint_cache/redis.py` (we are redis-stripped), and edits `config.example.yaml`. Exactly the shape of the trap in item 9 ("a new-package commit is NOT automatically additive — diff the SHARED files"). **INERT for us, verified four independent ways:** (1) the gate is real in code, not just the comment — `async_provider.make_checkpointer` wraps only `if mode == "delta"`, and its fallback when nothing is frozen is literally `"full"`; **our runtime mode is `full`**. (2) `config/database_config.py` is **PURELY ADDITIVE — 0 removed/changed lines**, so no pre-existing default moved. (3) the redis client is imported **lazily inside `_create_client`**, never at module level, and `REDIS_URL` / `DEER_FLOW_CHECKPOINT_CACHE_REDIS_URL` are unset in both `.env` and the container. (4) ⚠ **the decisive check — `database.checkpoint_cache` DOES now materialize with its default `type: memory`** (the new `CheckpointCacheConfig` has defaults), which reads alarmingly in a config dump; so rather than reason about it I asserted the live object: **`make_checkpointer(cfg)` yields a raw `AsyncPostgresSaver`, NOT a `CachedHistorySaver`.** 📌 **A materialized config default is NOT evidence a feature is active — instantiate the real object.**
- **`config_version` 31→32** — `_check_config_version` only WARNs when the user's version is lower; we have sat at `config_version: 9` for months, so this is a pre-existing no-op, not a new condition.
- **`#4647`** (keep inline-code `<think>` pairs in content, restore copy for reasoning-only turns) touches `core/messages/utils.ts` — **genuinely useful to us**: our qwen models emit `reasoning_content`, and a reasoning-only turn previously lost its copy button. **`#4634`** scopes artifact full-previews to their thread (a real cross-thread leak fix). **`#4639`**/**`#4643`** are composer/chat-sort UX. **`#4645`**/**`#4644`** are test-only — `#4645` is mildly notable in that a backend test was reading the real local `config.yaml`; test isolation only, no runtime effect.
- **Verified (all green):** 3 containers, **0 tracebacks**, startup complete → **3b** `schema in sync`, head **`0010_run_cancel_request`**, `consolidation_enabled False`, `injection_enabled False`, `retrieval_adapter ''`, `authz.enabled False`, `scheduler.enabled False`, sandbox `1.11.0`, ownership unset→memory, **`checkpoint_channel_mode: full`**, **live checkpointer `AsyncPostgresSaver` (unwrapped)**, 7 models / `models[0]=deepseek-v4-pro` / **`qwen3.8-max` present** → **3c** thread-id accept(64)+reject(65), `chat()` ×3 `CHAT_OK`, **UAE e2e `UAE_THREAD_OK`**, sandbox bash `COUNT=3` + `2:beta`, **qwen3.8-max re-smoked (thinking off/on + a sandbox tool turn)** → landing 200 / 12 WRI markers / 0 DeerFlow / 0 Galaxy, `/login` + `/setup` 200 WRI-branded, 2918 `WRI AI` in the built `.next`, `/api/models` 401, `/docs` 404, loopback bind, public **401**.
- **Carried patches:** i18n 30/29, nav 0, hero 0 Galaxy, 3 deleted sections absent, no redis service, `.env` intact, and **all 3 instance-only `config.yaml` items** (both pins + the `qwen3.8-max` entry). Backup: `~/deerflow-syncbak-20260803-130337/`.

Earlier upstream sync: **2026-08-02 (later)** — **2 commits (`cd8825b0`→`c0f1cfef`), CLEAN merge (0 conflicts), MIGRATION-FREE (head stays `0010`), 6 files / +266−29.** Merge `32bed127`. The quietest range in weeks.

- **`#4633` `fix(channels)`: offload outbound attachment file IO** (feishu/telegram/wecom) — **inert: channels are `<none / disabled>` at runtime**, confirmed by loading the real config rather than reading `config.yaml`.
- **`#3396` `fix(frontend)`: hide stale follow-up chips while a turn is streaming** — composer-only (`input-box.tsx`). A genuine web-chat fix (a message sent before the previous response finished left the old chips overlapping the input box, and a user-stopped turn no longer generates follow-ups). The LGI pipeline uses the embedded `chat()` path, so it never sees this.
- **Clean merge-tree was CORROBORATED, not trusted** (per the `#4618` lesson): `git diff main..local-fixes` is **empty for all 4 touched source files**, so none of them carries a WRI patch and there is no re-skin surface for a semantic conflict to hide in. Yesterday's conflict surfaces (`landing/`, i18n locales) are not in the range at all. No `.env`/compose/`config.example.yaml` changes either, so neither the `PORT`-expansion trap nor the silent-default rule applies.
- **Verified (all green):** 3 containers up, **0 tracebacks**, `Application startup complete` → **3b** `schema in sync — no missing columns`, head **`0010_run_cancel_request`**, `consolidation_enabled False`, `injection_enabled False`, `retrieval_adapter ''`, `authz.enabled False`, `scheduler.enabled False`, sandbox pinned **`1.11.0`** → **3c** thread-id accept(64)+reject(65), `chat()` ×3 **`CHAT_OK`**, **UAE e2e `UAE_THREAD_OK`** (the `#4589` regression guard — now a permanent 3c assertion), sandbox bash **`COUNT=3` + `2:beta`** → landing 200 / **12 WRI stat markers, 0 DeerFlow, 0 Galaxy**, `/login` + `/setup` 200 WRI-branded, **2918 `WRI AI` in the built `.next`**, `/api/models` 401, `/docs` 404, loopback-only bind, public **401** through Caddy.
- **Carried patches:** i18n **30 en / 29 zh**, nav 0 hits, hero 0 Galaxy, 3 deleted sections still absent, no redis service, `.env` + both `config.yaml` pins intact. Instance backup: `~/deerflow-syncbak-20260802-020307/`.

Earlier upstream sync: **2026-08-02** — **10 commits (`8234370a`→`cd8825b0`), ⚠ 6 CONFLICTS (all our carried WRI landing re-skin vs `#4622`), MIGRATION-FREE (head stays `0010`), 153 files.** Merge `5012a0e0`.

- ⚠⚠ **`#4589` thread-id validation BROKE THE LGI PIPELINE — caught and fixed BEFORE merging.** New `backend/packages/harness/deerflow/utils/thread_id.py` enforces `^[A-Za-z0-9_-]{1,64}$` via `validate_thread_id()`, called from `DeerFlowClient._stream_without_trace_context()` — i.e. the path **both `chat()` and `stream()` take** (`chat()` → `stream()` → `_stream_without_trace_context()`). **Confirmed empirically, not by code reading:** ran the real upstream module against **320 recorded thread ids** → **315 pass / 5 fail, every failure United Arab Emirates** (65–66 chars, because `lgi-pipeline-{iso}-united-arab-emirates-{model}-{epoch}` overruns). It fails **every run, deterministically**, and raises `ValueError` on the first turn — loud, not silent, so the blast radius is one country's Stage 1, re-runnable after a fix. Fixed in `~/lgi-pipeline` **first** (`7c62f99`): trim the **country slug, never the tail** (the epoch is what makes each launch a fresh thread; the model name separates concurrent model runs). Re-verified with the real validator: **126/126 ids (42 economies × 3 models) pass, max length exactly 64, 124 byte-identical to the old ones, no slug-prefix collisions**. **Post-sync end-to-end: a real `chat()` on the exact 64-char UAE id returned `UAE_THREAD_OK`, and the 65-char form raises `ValueError` — both directions asserted against the running gateway.** 📌 **LESSON: a new upstream validator is a contract change for every EXTERNAL caller — replay real recorded inputs through it before merging, don't just read the regex.**
- ⚠ **6 conflicts, all our carried landing re-skin vs `#4622` (perf: bound delivery/bundles/long-running UI).** Resolved **keep-OURS**, and unusually the upstream grafts had **nothing to attach to** — verified file by file rather than assumed: `page.tsx` (upstream's `<Header locale={DEFAULT_LOCALE}>` exists to feed `getI18n`, which our header never calls → passing it would be a dead prop, so **not** grafted); `header.tsx` (ours is a **sync** server component with no i18n at all — the WRI header is always English — so `getI18n(locale ?? DEFAULT_LOCALE)` is inapplicable); `hero.tsx` (ours is a static gradient + WRI stats with **no Galaxy and no FlickeringGrid**, so the `dynamic(ssr:false)` + `useRenderActivity` gating has no target). The 3 deleted sections (`case-study`, `skills`, `whats-new`) resolved by **`git rm`** — modify/delete, they stay deleted. ⚠ **`landing/sections/` is now a recurring conflict surface alongside the i18n locales.**
- **`#4540` authz Phase 3 (model authorization at Gateway routes + runtime) is INERT for us — verified in code, not from the changelog.** `_authorize_model_name()` opens with `if authz_config.enabled is not True: return model_name`, and we set **no `authorization:` block** (`authz.enabled: False` confirmed at runtime). Its one unconditional line — resolving `model_name=None` to `models[0].name` — **matches `create_chat_model`'s own `if name is None: name = config.models[0].name`**, so it changes nothing; and the LGI pipeline passes `model_name` explicitly on **both** turns anyway. It does touch `client.py`, so `chat()` was re-smoked ×3.
- **`#4617` MCP stdio arg/env constraints: write-path only.** Every `HTTPException` lives in `update_mcp_configuration` / `update_mcp_server_state` / `_validate_mcp_update_request` — validation runs when config is **POSTed**, never at load, so a running server is untouched. Moot regardless: **no `mcp_config.json` exists on this instance** (`cfg.mcp` is `None`).
- **`#4623` sandbox audit middleware is a net RELAXATION** — command substitution is now judged by *position*, so `x=$(curl u)` / `echo $(curl u)` (value position, output capture only) pass, while command position, `eval`/`source`, and interpreter code-string flags block. ⚠ **It has NO config gate** (`_build_runtime_middlewares` appends it unconditionally, lead agent + subagents), so it was smoked directly: sandbox bash `n=$(wc -l < f)` → **`COUNT=3` + `grep -n beta` → `2:beta`**, i.e. value-position substitution is not blocked.
- **Verified (all green):** build OK → `Application startup complete`, 0 boot errors, 3 containers up → **3b** `schema in sync — no missing columns`, alembic head **`0010_run_cancel_request`** unchanged, `consolidation_enabled False`, `injection_enabled False`, `retrieval_adapter ''`, `authz.enabled False`, `scheduler.enabled False`, sandbox image pinned **`1.11.0`**, `sandbox.ownership` unset → effective **memory** mode (single-instance — this is what validates the redis-strip), provider `AioSandboxProvider`, 6 models / `models[0]=deepseek-v4-pro` → **3c** thread-id accept+reject, `chat()` ×3 **`CHAT_OK`** (kimi-k3 / qwen3.7-plus / deepseek-v4-pro), UAE e2e **`UAE_THREAD_OK`**, sandbox bash + grep OK → landing 200 with **12 WRI stat markers, `bg-white`, 0 DeerFlow, 0 Galaxy, 0 deleted sections**, `/login` + `/setup` 200 and still WRI-branded with 0 DeerFlow, **`WRI AI` present in the built `.next` chunks** (bundle, not just source).
- **Carried patches verified post-merge:** i18n **30 en / 29 zh** `WRI AI` with only the 3 deliberate `officialWebsite`/`githubTooltip`/`visitGithub` DeerFlow credits; nav de-branding 0 hits; redis-strip intact (only 3 *comment* mentions in `docker-compose.yaml`, no service); `.dockerignore`, `backend/Dockerfile`, `.env` (`PORT=2026`, `UV_EXTRAS=postgres`, `GATEWAY_ENABLE_DOCS=false`) and both `config.yaml` pins unchanged. Instance backup: `~/deerflow-syncbak-20260802-010014/`.

Earlier upstream sync: **2026-08-01** — **13 commits (`133a82c6`→`8234370a`), CLEAN merge (0 conflicts), MIGRATION-FREE (head stays `0010`), 113 files / +6934−625.** Merge `bb357783`. Reviewed with a 40-agent risk analysis (27 CONFIRMED / 4 OVERSTATED / 4 UNDERSTATED).

- ⚠⚠ **`#4618` REQUIRED AN `.env` CHANGE AND `merge-tree` COULD NOT SEE IT — the conflict was SEMANTIC, not textual.** Upstream changed the compose port line to `"${BIND_HOST:-127.0.0.1}:${PORT:-2026}:2026"`. We bound to loopback via the trick **`PORT=127.0.0.1:2026`** (old OPS item 2), which under the new line expands to a **MALFORMED `127.0.0.1:127.0.0.1:2026:2026`** — `make up` would have failed and taken the site down. Upstream now does natively what the trick did, so **`.env` is now `PORT=2026` with `BIND_HOST` unset** (safe `127.0.0.1` default). **Verified post-merge: `deer-flow-nginx 127.0.0.1:2026->2026/tcp`, `ss` shows `127.0.0.1:2026` only, local `/` 200, `https://app.worldresearch.org/` 401 through Caddy.** ⚠ **Set `BIND_HOST=0.0.0.0` ONLY behind a TLS/auth front door — the agent can execute commands.** `.env` backup: `scratchpad/env.bak-20260801`. **LESSON: a clean `merge-tree` exit does NOT mean a compose/env change is safe — read the changed line and expand it against our actual `.env`.**
- ⚠ **`#4604` adds a FAIL-CLOSED scope gate inside deermem `_apply_updates`, our LIVE extraction path.** A fact is stored only if the extraction model labels it `scope=user` + `durability=durable` + `authority=descriptive`; **a MISSING label is a rejection.** Expect extraction write volume to **drop sharply** — our corpus is visibly task-scoped, and that is the intent. **Existing facts are NOT migrated/backfilled/pruned/reformatted** (`storage.py` untouched across the range) — **verified post-sync: 45 facts on disk, all still `schemaVersion: 2`.** We are NOT exposed to the un-migrated-template failure mode (`prompts_dir` unset → bundled prompt). Residual risk is model compliance: if kimi-k3 omits the labels every fact is rejected — **but injection is OFF, so facts are write-only and nothing consumes them: a tolerable degradation, NOT a rollback trigger.** 📌 **WATCH ITEM (not yet exercised):** the smoke's trivial turns produce nothing extractable, so the gate has NOT fired yet — after real usage, grep the gateway log for `Memory scope-gate metrics` / `scope-gate rejection rate`. ~100% rejection means the model isn't emitting the labels.
- **`#4178` rewrites AioSandboxProvider skills mounts onto a materialized `skills_view/` projection** — our provider, our acquire path. **Public skills are NOT hidden** (empty `skills` map = default-ENABLED) and the `ls`/`read_file`/`grep` path is untouched, but `/mnt/skills/legacy` and `/mnt/skills/integrations` now mount **unconditionally** and a projection failure fails **CLOSED** (empty `public` while the prompt still lists skills). **Verified post-sync: `ls /mnt/skills` → `custom integrations legacy public`, and `public` is POPULATED** (`academic-paper-review bootstrap chart-visualization`). Adds per-acquire projection work on the hot path.
- **`client.py`: ONE hunk, inside `update_skill()`.** AST-extracted `chat`, `stream`, `__init__` and every event emitter hash **IDENTICAL** before/after → event contract, per-id dedup and the returned string byte-for-byte unchanged.
- **Both carried-patch files verified via the MERGED BLOB, not the upstream diff:** merged `scripts/deploy.sh` has **zero redis refs** plus every upstream addition (diff vs upstream = exactly our one `services=` line); merged `docker-compose.yaml` has no redis service, no `redis-data` volume, no `DEER_FLOW_STREAM_BRIDGE_REDIS_URL`, no `depends_on`. **i18n is a PURE ADDITION** (2 new key groups) — merged blobs hold exactly **30 en / 29 zh** `WRI AI` lines and the 3 deliberate `DeerFlow` credits; **no re-skin file touched**.
- `#4590` seed-checkpoint-history writes only append-only `run_events` rows, never checkpointer tables, and our LGI threads are unreachable from it (embedded path). `#3183` context-window usage adds no tokenizer, no extra LLM call (`models[*].context_window` code default `None` == our unset). `#4596`'s new artifact-write route provably skips the sandbox branch for our config and does not bypass the read-before-write gate. `#4575` DOES touch shared `aio_sandbox/ownership/factory.py` — flagged, checked, benign. `#4607` scheduler + `#4598` wizard inert.
- **Verified (all green):** `✓ Compiled successfully in 58s` → `upgrade head (0010)`, 0 boot errors, no extra containers → 3b `schema in sync`, consolidation `False`, retrieval `''`, **45 facts / `schemaVersion: 2` intact** → **3c** sandbox `838c7068` `SYNC801_SANDBOX_OK` + `GREP_DIR`/`GREP_FILE` + `READ_RANGE: OK` → `chat()` ×3 `CHAT_OK` → `/login` 17/0, `/setup` 10/0, landing 25 `WRI AI`, `/docs` 404, `/api/models` 401. Rollback tag `pre-sync-20260801` → `d24777d2` (⚠ also restore `.env` PORT from `scratchpad/env.bak-20260801`).

Prior sync **2026-07-31** — **4 commits (`0d8e11ad`→`133a82c6`), CLEAN merge (0 conflicts), MIGRATION-FREE (head stays `0010`), 29 files / +1016−140.** Merge `9753bda0`. Verified inline.

- **`#4587` security: harden auth next-path validation.** New `frontend/src/core/auth/next-path.ts` rejects a post-login `next` that doesn't start with `/`, starts with `//` (protocol-relative → external host), or contains `\` or `:` — classic open-redirect payloads — falling back to `/workspace`. ⚠ **It edits `(auth)/login/page.tsx`, our recurring conflict surface**, so rather than trust `merge-tree`'s exit code I extracted the MERGED BLOB and checked it: maroon `#7b1e2b` ×5 and `setupStatusPhase` ×6 (both matching `local-fixes`), **0 deerflow refs**, AND their new `next-path` import present. **The 3-way merge kept OURS-skin and grafted their fix — no manual resolution needed** (first time this surface merged clean rather than conflicting). Re-confirmed on disk post-merge.
- **`#4577` touches `client.py`** — but only `_atomic_write_json`, the MCP-config writer, now delegating to the shared `atomic_write_extensions_config` (same file-locking work as `#3565`). **`chat()` and `stream()` are BYTE-IDENTICAL across the range** (md5-compared before/after), so the embedded LGI path is untouched. `#4582` MCP oversized path-like text; `#4584` frontend artifact refresh.
- ⚠ **TESTING NOTE — my first `#4587` check was worthless and I corrected it.** I curled `/login?next=<payload>` and got **200 for every payload**, including `//evil.example.com` — because `/login` renders the page regardless; the validation runs at REDIRECT time, not page load. An assertion that cannot fail. **The real gate is the pure function**: extracted `validateAuthNextPath`/`resolveAuthNextPath` and ran 9 payloads — `/workspace` + `/account/library` ALLOW; `//evil.example.com`, `https://evil…`, `javascript:alert(1)`, `\\evil…`, `/\evil`, bare `workspace`, and `""` all BLOCK and resolve to `/workspace`. **9/9.** Same lesson as the paywall-marker and hydration bugs: assert on the thing that actually decides, not on a page that renders either way.
- **Verified (all green):** `✓ Compiled successfully in 53s` → `upgrade head (0010)`, 0 boot errors, no extra containers → 3b `schema in sync`, consolidation `False`, retrieval `''`, no index file, **MCP config intact (`['playwright']`)** → **3c** sandbox `784394a4` `SYNC731_SANDBOX_OK` + `GREP_DIR`/`GREP_FILE` OK + `READ_RANGE: OK` → `chat()` ×3 `CHAT_OK` → `/login` 17/0, `/setup` 10/0, `/docs` 404, `/api/models` 401. Carried patches + both pins intact. Rollback tag `pre-sync-20260731` → `5c238658` (drop post-push; plain `git reset`).

Prior sync **2026-07-30** — **13 commits (`4582a413`→`0d8e11ad`), CLEAN merge (0 conflicts), MIGRATION-FREE (head stays `0010`), 73 files / +4560−385.** Merge `485a76d7`. Reviewed with a 25-agent risk analysis (15 CONFIRMED / 3 OVERSTATED / 2 UNDERSTATED) **plus a LIVE IMAGE PROBE of the one medium.**

- ⚠⚠ **`#3824` pushes `read_file` ranges SERVER-SIDE with NO FALLBACK** — `aio_sandbox.py` passes `start_line`/`end_line` straight to the SDK's `v1/file/read`, wrapped only in `except Exception → return f"Error: {e}"`. **Upstream wrote a 404 capability probe for `bash.exec`/1.9.3 and did NOT write one here.** MILDER than `#4512` (which moved grep to a NEW route our frozen image 404'd): this adds two OPTIONAL fields to an endpoint we already call every turn, and the SDK omits them when `None`, so **non-ranged reads are wire-identical and cannot regress**. But image support was **not determinable from the repo**, and the harness actively steers the model into ranged reads (`tools.py:1618` emits *"Use start_line/end_line to read a specific range"* on any >50k read) — so this is NOT opt-in. **PROBED THE LIVE 1.11.0 IMAGE BEFORE MERGING** (`scratchpad/rangeprobe.py`, calling the SDK directly): full=60 lines, `(0,5)`→`line1..line5`, `(10,15)`→`line11..line15` — **0-based start, exclusive end, exactly as documented. HONOURED → safe as-is.** ✅ **The ranged read is now a PERMANENT part of the 3c smoke** (`READ_RANGE`), same as grep after `#4512`. Had the image ignored the fields the failure would have been SILENT (whole file returned, head-truncated at 50 000 chars). ℹ️ The `read_before_write` gate is unaffected either way — it does its OWN unranged read (`tools.py:2125`), so the stamped hash is always the full-file hash (pinned upstream by `test_ranged_read_stamps_full_file_hash`).
- **`#4384` "route subagents by net benefit" is PROMPT TEXT ONLY** — no routing code, no middleware, no tool-arg schema. Fully gated on `subagent_enabled`, so the **embedded LGI path gets a byte-identical system prompt and tool list**. Ultra mode's fan-out flips from default-on to opt-in (should reduce subagent count/tokens).
- **`#3442` custom postgres schema — inert.** `postgres_schema` code default is `""` and we set only `backend` + `postgres_url`: no `search_path` SET, no schema-qualified DDL, no `CREATE SCHEMA`, no existence check. A verifier ran the new `normalize_libpq_dsn()` against our real DSN — returns the identical string. (Our legacy `checkpointer:` block stays authoritative for the checkpointer and also defaults to `""`.)
- **`#4248`/`#4505`** teardown-ordering fix is reachable ONLY after readiness already failed — timeout stays 60 s, success path untouched, so acquire latency and cold starts are unchanged. **`#4570` is NOT inert and is a free win:** it fixes a live latent `AttributeError` on the slash-skill activation path in our tree. `#4564` cost-reporting guard can't trip (we set no `pricing` blocks). `#4452` scheduler inert. `#4482` test-only.
- **Frontend: only THREE production files change and ZERO are under `frontend/src/core/`** — the i18n rebrand and every re-skin file are byte-identical in the merged tree. `#4580` persists panel state to **sessionStorage**, not the DB (no migration).
- **Verified (all green):** `✓ Compiled successfully in 62s` → `upgrade head (0010)`, 0 boot errors, no extra containers → 3b `schema in sync`, consolidation `False`, retrieval `''`, no index file → **3c** sandbox `c6d7ca84` `SYNC730_SANDBOX_OK` + `GREP_DIR`/`GREP_FILE` OK + **`READ_RANGE: OK` (full=60, ranged=5, first `line11`)** → `chat()` ×3 `CHAT_OK` → `/login` 17/0, `/setup` 10/0, landing 25 `WRI AI`, `/docs` 404, `/api/models` 401. Carried patches + both pins intact. Rollback tag `pre-sync-20260730` → `40c7a67a` (drop post-push; plain `git reset`).

Prior sync **2026-07-29 (later)** — **6 commits (`4e449385`→`4582a413`), CLEAN merge (0 conflicts), MIGRATION-FREE (head stays `0010`), 24 files / +1261−175.** Merge `9b762282`. Verified inline. **`client.py` untouched (0 matches in range); root `.dockerignore` untouched** — ⚠ the carried-patch grep flagged a `dockerignore` hit but it was the NEW `docker/lark-cli-broker/Dockerfile.dockerignore` for an image we never build (check WHICH file the pattern matched, not just that it matched). `merge-tree` exit 0.

- **`#3565` offload blocking FS IO in `update_skill` + serialize writes — the one that applies to us.** It moves the read-modify-write of `extensions_config.json` off the event loop, and adds a `threading.Lock` **acquired inside the worker** because the offload alone would introduce a lost update: the skills router and the MCP router both RMW that same file, and once a writer is on a worker thread the loop can interleave the other writer inside the read→write window, silently dropping the first's change. Deliberately NOT an `asyncio.Lock` — one held around `await asyncio.to_thread(...)` releases on cancellation while the worker keeps writing. Relevant here because we run **one MCP server (playwright)** and skills in that same file. Same lost-update family as `#4489`'s `threads_meta` row-lock.
- `#4410` batch main-thread stream updates (frontend `hooks.ts`) — perf win for the web chat; `#4559` one workspace-change card per run; `#4163` stamp elapsed duration once per run; `#4556` panel drag-collapse state; `#4558` CI-only (publishes lark-cli-init/broker images we never build).
- **Verified (all green):** `✓ Compiled successfully in 53s` → `upgrade head (0010)` (no new migration), 0 boot errors, no extra containers → 3b `schema in sync`, consolidation `False`, retrieval `''`, no index file, **plus an `#3565`-specific check: `extensions_config.json` still valid JSON on disk and `ExtensionsConfig.from_file()` loads `mcp_servers: ['playwright']`** (⚠ the field is `mcp_servers`, snake_case — the JSON key is `mcpServers`) → **3c** sandbox `a0184bda` `SYNC729B_SANDBOX_OK` + `GREP_DIR`/`GREP_FILE` OK → `chat()` ×3 `CHAT_OK` → `/login` 17/0, `/setup` 10/0, `/docs` 404, `/api/models` 401. Carried patches + both pins intact. Rollback tag `pre-sync-20260729b` → `64960200` (drop post-push; plain `git reset`).

Prior sync **2026-07-29** — **8 commits (`c24bf383`→`4e449385`), CLEAN merge (0 conflicts), MIGRATION-FREE (head stays `0010`), 47 files / +2656−194.** Merge `47566ec6`. Verified inline. ⚠ **Recon lesson: my first-pass gates called `#4528` (mem0 backend) "not our backend" — but it REFACTORS `memory_middleware.py`, our every-turn path** (`after_agent` split into `_resolve_add_args` + a new async `aafter_agent`; same queue-write, properly async on async graphs) and adds a `ClassVar` + non-constructing resolver to `manager.py`. **A "new backend" commit is not automatically additive — always diff the SHARED files in the range, not just the new package dir.** Resolved benign: 0 new packages in `uv.lock`, and the 3c chat smoke exercises the refactored write path directly (3 chats → 0 memory errors). Other items: `#4429` moves the sandbox `Overwrite`-unwrap helper to `sandbox/overwrite.py` + applies it to three read-only `tools.py` call sites (defensive, our path); `#4521` tool-mode recall is gated on `mode=="tool"` (we run `middleware`, injection off — double-inert); `#4405` pnpm/Corepack wrapper touches host-side Makefiles only (`deploy.sh` untouched; Docker builds unaffected); `#4552` OpenViking hardening (not our backend); 3 dependabot bumps (setuptools 83 / pyasn1 / postcss 8.5.24, version-only). `config.example` additions are comments only. **client.py, i18n and every re-skin file untouched.** **Verified (all green):** `✓ Compiled successfully in 64s` → `upgrade head (0010)`, 0 boot errors, no extra containers → 3b `schema in sync`, consolidation `False`, retrieval `''`, no index file → **3c** sandbox `57bfea63` bash + `GREP_DIR`/`GREP_FILE` OK → `chat()` ×3 `CHAT_OK` + **0 memory-subsystem errors through the refactored middleware** → `/login` 17/0, `/setup` 10/0, `/docs` 404, `/api/models` 401. Carried patches + both pins intact. Rollback tag `pre-sync-20260729` → `833774d9` (drop post-push; plain `git reset`).

Prior sync **2026-07-28 (later³)** — **10 commits (`9a43d827`→`c24bf383`), CLEAN merge (0 conflicts), MIGRATION-FREE (head stays `0010`), 116 files / +6947−1177.** Merge `8f86b91f`. Reviewed with a 21-agent risk analysis (15 CONFIRMED / 1 UNDERSTATED-nit). **`docker-compose.yaml` was touched for the FIRST time since the redis-strip and merged CLEAN** — upstream's whole delta is two env-line hunks (`openviking` appended to the gateway `NO_PROXY` hostname list from `#4509`; `LARK_CLI_BROKER_IMAGE=${LARK_CLI_BROKER_IMAGE:-}` default-empty on the **provisioner** service from `#4501`). **No new service/volume/depends_on; `scripts/deploy.sh`, `backend/Dockerfile` and `Makefile` unchanged.** The OpenViking service lives only in the new opt-in `-f` overlay `docker-compose.openviking.yaml` (on no Makefile/deploy path — do NOT add it); the lark broker is K8s-provisioner-only, its probe triple-gated off for us (no `provisioner_url`, no Lark pack, no `lark-cli` commands). **`client.py`: one commit (`#4516`), 6 constructor-plumbing hunks — `chat()`/`stream()`/event contract byte-identical**; new nested key `database.checkpoint_delta.snapshot_frequency` (example 10 == code default 10, delta-only; the old code-only 1000 default is gone — irrelevant in our `full` mode). **`#4538` identity: verified empirically post-sync — embedded no-identity `get_effective_user_id()` still returns `'default'`**, so the LGI on-disk layout under `users/default/` is unmoved. `#4509` OpenViking backend is additive (deermem diffs EMPTY; the backend registry eagerly imports the package but it is definition-only, httpx already core, no lockfile change). `#4536` `sandbox.thread_data_mounts` default `None` = behaviour identical (`config_version` 30→31). `#4535` adds only `Access-Control-Expose-Headers: Content-Location`. i18n +1 key (`sandboxRuntimeBroker`), far from the rebrand. ⚠ **WATCH ITEM `#4497` (Ultra mode):** parent `RunJournal` callbacks no longer leak into the isolated subagent loop (fixes duplicate event accounting + "Future attached to a different loop" — net positive), **but subagent skills are now LAZY** — SKILL.md bodies are no longer eagerly injected into subagent prompts and the static allowed-tools clamp is replaced by the lead-style activation middlewares. If Ultra-mode output changes character around skills, this is why. **Verified (all green):** `✓ Compiled successfully in 61s` → `upgrade head (0010)` (no new migration), 0 boot errors, **NO redis/provisioner/openviking containers** → 3b `schema in sync`, consolidation `False`, retrieval `''`, no index file, **identity `'default'` confirmed** → **3c** sandbox `4eb39a68` bash + `GREP_DIR`/`GREP_FILE` OK → `chat()` ×3 `CHAT_OK` → `/login` 17/0, `/setup` 10/0, `/docs` 404, `/api/models` 401. Carried patches + both instance pins intact. Rollback tag `pre-sync-20260728d` → `68371b2b` (drop post-push; migration-free — plain `git reset`).

Prior sync **2026-07-28 (later²)** — **6 commits (`88f9f818`→`9a43d827`), CLEAN merge (0 conflicts), 53 files / +3685−223, ⚠ MIGRATION `0009`→`0010_run_cancel_request`.** Merge `e38ec466`. Verified inline (scoped checks — migration read in full, security fix traced to its predicate, cancel gating confirmed in the diff):

- **`0010_run_cancel_request` (`#4500`)** — two nullable `runs` columns (`cancel_action`, `cancel_requested_at`) via the idempotent `safe_add_column` helpers; clean `downgrade()`. Applied at bootstrap (`Running upgrade 0009_webhook_dedupe -> 0010_run_cancel_request`), **verified via SQL: `alembic_version=0010_run_cancel_request`, both columns present.** ⚠ **Code rollback past this sync needs `alembic downgrade 0009_webhook_dedupe`.** The cross-worker cancel machinery itself is **gated on `heartbeat_enabled`** (False for us — the gating conditionals are visible in the diff) → single-worker cancel behaviour unchanged.
- **`#4448` security hardening — ZERO exposure for us, taken gladly:** Gateway run admission now **422-rejects any run whose metadata carries the legacy `auth_token` key** (`secret_context.py::validate_run_metadata_secrets`) and `redact_metadata_secrets` strips it from API-returned historical metadata; callers are pointed at `config.context.secrets`. Our frontend never sends it, the embedded LGI path bypasses Gateway admission entirely, and **our single MCP server (playwright) is stdio with `env: {}`** — no credentials anywhere.
- **`#4534`** edit-rerun replay from a settled checkpoint (Gateway-only); **`#4510`/`#4525`** frontend streaming step-text/list-marker stability (web-chat wins); **`#4537`** vLLM cumulative stream usage — our only vLLM refs are **comments** (config.yaml:501-504) → inert, and the `config.example.yaml` delta is **2 comment lines** (no new keys).
- **`client.py`, i18n locales and every re-skin file untouched; `merge-tree` exit 0.** Carried patches + both instance pins verified intact post-merge.
- **Verified (all green):** `✓ Compiled successfully in 55s` → migration applied, 0 boot errors, NO redis/provisioner → 3b `schema in sync`, consolidation `False`, retrieval `''`, no index file → **3c** sandbox `25b008b2` bash + **`GREP_DIR`/`GREP_FILE` OK** → `chat()` ×3 `CHAT_OK` → `/login` 17/0, `/setup` 10/0, `/docs` 404, `/api/models` 401. Rollback tag `pre-sync-20260728c` → `c782b0a8` (drop post-push; ⚠ + `alembic downgrade 0009_webhook_dedupe`).

Prior sync **2026-07-28 (later)** — **5 commits (`6456c356`→`88f9f818`), CLEAN merge (0 conflicts), MIGRATION-FREE (head stays `0009`), 22 files / +1105−55, frontend-heavy.** Merge `b55066e4`. Small enough to verify INLINE (three targeted checks) rather than a full agent fan-out: **(1)** the one new `config.example.yaml` key `suggestions.max_suggestions: 3` has CODE default `3` (`suggestions_config.py::DEFAULT_MAX_SUGGESTIONS`) — equal, and we set no `suggestions:` block → no silent-default divergence; **(2)** `#4527`'s `ask_clarification` change is pure input normalization (flattens dict-shaped option payloads, strips XML tags from option text) on the middleware chain — LGI never elicits clarifications, web chat just gets cleaner options; **(3)** `merge-tree` exit 0, and **no migrations / no `client.py` / no i18n locales / no re-skin files** — the quietest batch on carried-patch surfaces in weeks. Batch: `#4513` preserve message order during long runs (+302 in `threads/hooks.ts` message-merge — a real web-chat fix), `#4486` citation links from React children, `#4539` keep manual rename through edit-and-rerun (Gateway router only), `#4533` suggestions count, `#4527` above. **Verified (all green):** `✓ Compiled successfully in 49s` → bootstrap `upgrade head (0009_webhook_dedupe)` (no new migration), 0 boot errors, NO redis/provisioner → 3b `schema in sync`, consolidation `False`, retrieval pin `''` + no index file, both config pins intact (`retrieval_adapter`/`sandbox.image 1.11.0`) → **3c** sandbox `9aa851c9` `SYNC728B_SANDBOX_OK` + **`GREP_DIR`/`GREP_FILE` OK** (the grep smoke is now standard) → `chat()` ×3 `CHAT_OK` → `/login` 17/0, `/setup` 10/0, `/docs` 404, `/api/models` 401. Rollback tag `pre-sync-20260728b` → `0209374c` (drop post-push; plain `git reset` — migration-free).

Prior sync **2026-07-28** — **12 commits (`62b73fd2`→`6456c356`), CLEAN merge (0 conflicts), 93 files / +7497−465, ⚠ FIRST MIGRATION since 0008: head `0008`→`0009_webhook_dedupe`.** Merge `ef2f4f27`. Reviewed with a 29-agent risk analysis (20 CONFIRMED / 1 OVERSTATED / 3 UNDERSTATED). **Two instance-config changes shipped WITH this sync (both in the gitignored `config.yaml` — a droplet rebuild loses them, restore by hand):**

- ⚙ **`memory.backend_config.retrieval_adapter: ""` — PINNED OFF.** `#4360` (FTS5/BM25 retrieval) flipped the DeerMem CODE default `""`→`"fts5"`, a genuine silent-default flip for a gitignored config: unpinned, the next boot builds+maintains a derived sqlite index (`.deer-flow/.retrieval/memory-fts5.sqlite3`, startup rebuild + per-write upkeep). With `injection_enabled: false` + middleware mode, retrieval SEARCH never runs here → the index would be pure derived overhead, so we pin the pre-sync behaviour. **Verified:** effective `retrieval_adapter=''`, NO index file, consolidation still `False`. ℹ️ The startup line **"Memory retrieval index rebuilt successfully" still appears WITH the adapter disabled** — `warm_retrieval` returns True as a no-op when storage has no `rebuild_index` (`deer_mem.py:497-499`); benign, don't chase it. Remove the pin only if injection/tool-mode is ever enabled.
- ⚙ **`sandbox.image: ...all-in-one-sandbox:1.11.0` — PINNED (was unset → code-default `:latest`).** ⚠⚠ **THE 3C GREP SMOKE CAUGHT A REAL BREAK:** `#4512` rewrote `AioSandbox.grep` to the server-side `grep_files` endpoint with **NO fallback**, and our `:latest` image — **pulled 7 months ago and frozen** (the same "`:latest` is not latest" trap the code itself documents for `bash.exec`/1.9.3) — **404'd it**, breaking the agent's `grep` tool while bash still worked. Fixed: pulled `1.11.0` (upstream's config.example reference), pinned the tag, restarted; grep now returns matches for BOTH a directory root and a single-file root (the newly allowed case). **Standing rule: when a sync moves a sandbox tool onto a new server endpoint, the 3c smoke MUST exercise that tool** — bash-only smoke would have shipped this broken. When bumping the image: pull FIRST, then restart, then re-run the grep smoke.

Batch detail: **`#4210`** webhook dedupe — migration `0009` creates one empty `webhook_deliveries` table + index (purely additive, idempotent `has_table` guard, clean `downgrade()`); the dedupe store IS constructed at startup (`dedupe_storage=auto` INFO line is expected) but is lazily inert with zero channels; table verified present + 0 rows via SQL. ⚠ **Code rollback past this sync needs `alembic downgrade 0008_thread_operation_kind`.** `config_version` 29→30 (`dedupe_storage` code default == example value → no divergence). **`#4494`** artifact-delivery enforcement is **ALWAYS-ON**: a Gateway/UI run that writes `/mnt/user-data/outputs` files without calling `present_files` now ends **`status=error` ("Artifact delivery incomplete")** — ⚠ **if an output-producing web-chat run streams fine but ends error, it's THIS, not infra** (embedded LGI path unaffected — no RunJournal). **`#4489`** row-locks `threads_meta` merges (`SELECT … FOR UPDATE`) — net-positive, closes a real lost-update race for the web chat. **`#4377`** edit-and-rerun is Gateway+frontend only. Tenki additive (optional extra, not in `UV_EXTRAS`); browserless/crawl4ai unconfigured; postcss bump = lockfile no-op (real delta: dev-only rollup). **`client.py`: ZERO commits in the range.** Carried patches all intact (i18n 30/29 + 4 new keys in all 3 locale files, none saying "DeerFlow"; redis-strip 0/0; `.env` pair; Dockerfile ×8; login graft ×6; nav 0). **Verified (all green):** `✓ Compiled successfully in 53s` → bootstrap **`Running upgrade 0008_thread_operation_kind -> 0009_webhook_dedupe`** → 0 boot errors, NO redis/provisioner → `alembic_version=0009_webhook_dedupe` + `webhook_deliveries` 0 rows (SQL) → 3b `schema in sync`, consolidation `False`, retrieval `''`, no index file → **3c** sandbox `a90433a7` `SYNC728_SANDBOX_OK` + **`GREP_DIR: OK` + `GREP_FILE: OK` on 1.11.0** → `chat()` ×3 `CHAT_OK` → `/login` 17 WRI / 0 deerflow, `/setup` 10/0, `/docs` 404, `/api/models` 401. Rollback tag `pre-sync-20260728` → `0a2ee4ad` (drop post-push; ⚠ + `alembic downgrade 0008`).

Prior sync **2026-07-27 (later)** — **2 commits (`b22f85c6`→`62b73fd2`), CLEAN merge (0 conflicts), MIGRATION-FREE (head stays `0008`), 11 files / +1721−11, backend-only.** Merge `35d5d236`; local `main` marker → `62b73fd2` (**not pushed** — see the fork-auto-sync note above).

- ⚠ **`#4439` `6091ce75` derive Gateway route permissions from AuthorizationProvider — touches `auth_middleware.py`, which runs on EVERY authenticated request.** It replaces a constant with an await: `request.state.auth = AuthContext(user=user, permissions=_ALL_PERMISSIONS)` becomes `permissions = await resolve_route_permissions(user, is_internal=...)`. **Proven equivalent for us, empirically — not by code reading:** `resolve_route_permissions` (`app/gateway/authz.py:205-207`) opens with `config = _get_route_authorization_config()` / `if config.enabled is not True: return list(_ALL_PERMISSIONS)`, i.e. it early-returns *before* provider resolution, principal construction, or any async permission evaluation. Our runtime is `authorization = enabled=False fail_closed=True default_role='user' provider=None`, and a **direct probe on the running image returned 6 permissions == `_ALL_PERMISSIONS` (6), sorted-equal `True`**. The config lookup is the already-cached `get_app_config().authorization` (`authz.py:139-150`) — the same object Gateway deps read every request — so **no new I/O on the hot path**; the only delta is one list allocation per request instead of passing a constant. ⚠ If authz is ever ENABLED, note `fail_closed=True` is our default: a provider failure would then return `[]` (deny) rather than all-permissions.
- **`#4423` `62b73fd2` DingTalk inbound file/image attachments — INERT.** Confined to `app/channels/dingtalk.py` + tests (the only other file is `backend/AGENTS.md`). We configure **no `channels:` block at all**, so no DingTalk channel ever starts.
- **`config.example.yaml` is touched but its ENTIRE delta sits inside a commented-out `#   provider:` example block** (adding `routes:` alongside `tools:` in the RBAC role samples); the live `authorization: enabled: false` is unchanged. **No new keys → no silent-default exposure this sync.**
- **`client.py`, the i18n locales and every re-skin file are UNTOUCHED.** Carried patches verified intact: i18n **30/29**; the 2026-07-27 branding fixes still present (`WRI AI backend` ×2, `WRI AI chat` ×1, `WRI AI AI` ×0); redis-strip 0/0; `.env` `UV_EXTRAS=postgres` + `GATEWAY_ENABLE_DOCS=false`; Dockerfile ×8; login graft ×6; nav 0.
- **Verified (all green):** `make up` exit 0 (backend-only batch, so the frontend Next.js stage was CACHED — no `Compiled successfully` line is expected here, and its absence is not a failure) → bootstrap `upgrade head (0008_thread_operation_kind)`, **0 boot errors**, NO redis/provisioner → `/` `/login` `/setup` `/health` 200, `/docs` + `/openapi.json` still **404**, and **`/api/models` → 401 (not 500)** which is the direct check that `#4439`'s new per-request await did not break the auth path → **3b `schema in sync`**, `consolidation_enabled=False`, `checkpoint_channel_mode=full`, `heartbeat_enabled=False`, **`resolve_route_permissions` ≡ `_ALL_PERMISSIONS`** → **3c Aio smoke** sandbox `b8325947` `SYNC727B_SANDBOX_OK / Linux / gem` → **`chat()` kimi-k3 / qwen3.7-plus / qwen3.7-max all `CHAT_OK`**. Rollback tag `pre-sync-20260727b` → `11d9daf7` (drop post-push).

Prior sync **2026-07-27** — **8 commits (`244ce773`→`b22f85c6`), CLEAN merge (0 conflicts), MIGRATION-FREE (head stays `0008`), 77 files / +7769−539.** Merge `2a1ebe61`; `main` ff'd → `b22f85c6`. Reviewed with a 37-agent parallel risk analysis (5 dimensions, refute-by-default verify): **23 CONFIRMED / 8 OVERSTATED / 1 REFUTED, nothing medium or high.**

- ⚠ **`client.py` WAS touched — the first time in many syncs — and it is a NON-EVENT.** Exactly one commit does it (`e01173d8` `#4467`, the *benchmark* commit) and the diff is **2 lines**: an import, plus `freeze_delta_snapshot_frequency(self._app_config.database.checkpoint_delta_snapshot_frequency)` in `DeerFlowClient.__init__`. **`chat()`, `stream()`, the event constructors, the `messages-tuple` type values, the per-id dedup, the accumulation and the returned string are byte-identical.** The frozen global is read only behind `mode == "delta"`, which we never take. The LGI call site (`DeerFlowClient()` no-args, two `chat()` calls, one client per `docker exec` process) is unaffected — no new raise/hang/empty-return path. ⚠ Its two `raise` paths are unreachable for us: `frequency <= 0` is rejected by `Field(ge=1)` at config load, and the reconfiguration raise needs two freezes with DIFFERENT values in one process (we construct exactly one client per process).
- **`#4431` `090e80c1` fail-stop runs when lease ownership cannot be confirmed — UNREACHABLE for us.** Every path that sets the new `RunRecord.ownership_lost` fence is gated on `RunManager.heartbeat_enabled`; our `config.yaml` contains **zero** `run_ownership` entries so it falls back to `RunOwnershipConfig()` with `heartbeat_enabled=False`, and `start_heartbeat()` returns before creating the loop — so `_renew_leases()`, the only place that fences a *running* run, never executes. **Empirically confirmed at 3b: `run_ownership = lease_seconds=30 grace_seconds=10 heartbeat_enabled=False`.** It also never reaches the embedded path (`RunManager` is constructed only at `app/gateway/deps.py:458`; `client.py` imports nothing from `deerflow.runtime.runs`). **Its 5 added `config.example.yaml` lines are ALL COMMENTS — no new keys, no silent-default drift.** The one live change is `RunRepository.update_run_completion` becoming a conditional UPDATE (Postgres), which only alters a Gateway cancel-vs-complete race and cannot stop a run. ⚠ **STANDING ADVISORY: do NOT set `run_ownership.heartbeat_enabled: true` while `GATEWAY_WORKERS=1`** — this commit makes that combination newly hazardous.
- **`#4444` `2e5c8da2` bypass proxies for local AIO traffic — DOES land on our sandbox path, and is proven equivalent.** It adds a pure-function classifier `sandbox_http_trust_env()` and sets `trust_env=False` on the three clients that talk to the AIO control plane. Our sandbox URL is `http://host.docker.internal:<port>`, which matches the `.docker.internal` branch, so **we take the new direct path on every sandbox call**. **Verified live at 3b: `trust_env("http://host.docker.internal:8080") = False`, `trust_env("https://example.com") = True`.** Not a no-op by construction, but proven equivalent for us: with only our compose `NO_PROXY` set, httpx's `trust_env=True` path builds mounts that all map to `None` and returns the same transport. Residual deltas (`trust_env=False` also stops honouring `SSL_CERT_FILE`/`SSL_CERT_DIR`/`REQUESTS_CA_BUNDLE`/`.netrc`) are irrelevant on a plain-HTTP loopback-class URL where we set none of them. ⚠ **One finding was REFUTED here:** the claim that the new `requests.Session()` was for keep-alive is wrong — it exists *solely* so `session.trust_env` can be set. No lifecycle, timeout, ownership or config-key changes.
- **`#4426` `1cd5dea3` signal replay history gaps** — adds a genuinely NEW SSE control event `gap` plus a `StreamGap` dataclass on the **bridge-subscriber** path. It does NOT add, rename or alter any `values` / `messages-tuple` / `custom` / `end` payload, so `chat()`'s accumulation loop is untouched — **the embedded client never constructs or subscribes to a StreamBridge at all** (it iterates `self._agent.stream(...)` directly). It DOES touch our *memory* bridge (not redis-only) but only inside `subscribe()`, reached exclusively from `sse_consumer` / `wait_for_run_completion` / the IM-channels watcher — Gateway-HTTP paths (Web UI). Live consequence there: a subscriber that falls behind now terminates rather than silently skipping.
- **`#4406` `1baa8ad6` structured form fields for human-input cards** — ⚠ **the one change that genuinely reaches our embedded path**: it slightly enlarges the model-visible `ask_clarification` tool schema (adds a `fields` parameter). Legacy v1 wire format preserved. **`#4480`** is a single 9-line hunk in one regenerate-only Gateway route (does not touch `TitleMiddleware` or the worker fallback-title write; zero effect on fresh runs). **`#4443`** is genuinely E2B-only (`warm_pool_lifecycle.py` and `aio_sandbox/ownership/*` untouched across the whole range). **`#4467`/`#4484`** are bench + changelog.
- 📌 **New code-only key `database.checkpoint_delta_snapshot_frequency`** (`database_config.py:56`, default `1000`, `ge=1`) is **absent from `config.example.yaml`**, so we inherit the code default — which is **byte-equal to the previously hardcoded `DeltaChannel(..., snapshot_frequency=1000)`**. Inert (delta-only). This is the *benign* end of the gitignored-config silent-default class: the test is always **code default vs the value in `config.example.yaml`** — equal means no divergence.
- **Carried patches all intact:** i18n **30/29** `WRI AI`; the 4 new keys (`streamReplayGap` from `#4426`; `humanInput.requiredError`/`requiredA11yLabel`/`selectPlaceholder` from `#4406`) landed in `en-US.ts` + `zh-CN.ts` + `types.ts`; **yesterday's branding fixes survived** (`WRI AI backend` ×2, `WRI AI chat` ×1, `WRI AI AI` ×0); redis-strip 0 refs in compose and `deploy.sh`; `.env` `UV_EXTRAS=postgres` + `GATEWAY_ENABLE_DOCS=false`; Dockerfile ×8; login `setupStatusPhase` ×6; nav 0 deerflow refs.
- **Verified (all green):** `make up` exit 0, both images rebuilt → frontend **`✓ Compiled successfully in 51s`** (the real TS gate for the 4 new keys) → bootstrap `upgrade head (0008_thread_operation_kind)`, **0 boot errors**, **NO redis/provisioner**, `/` `/login` `/setup` `/health` 200 **and `/docs` + `/openapi.json` still 404** (the `GATEWAY_ENABLE_DOCS=false` from `.env` survived the rebuild) → **3b `schema in sync`**, `consolidation_enabled=False`, `queue_max_depth=1000`, `checkpoint_channel_mode=full`, `checkpoint_delta_snapshot_frequency=1000`, `heartbeat_enabled=False`, sandbox trust_env classifier correct → **3c Aio smoke** sandbox `56b920d2` `SYNC727_SANDBOX_OK / Linux / gem` (exercises the `#4444` path) → **`chat()` kimi-k3 / qwen3.7-plus / qwen3.7-max all `CHAT_OK`** → rebuilt bundle: 38 files with `WRI AI`, **0** with `WRI AI AI`, 6 with `WRI AI backend`; `/login` 17 WRI / 0 deerflow, `/setup` 10 WRI / 0 deerflow. Rollback tag `pre-sync-20260727` → `488fa421` (drop post-push).

Prior sync **2026-07-26 (later⁴)** — **7 commits (`04b7e693`→`244ce773`), CLEAN merge (0 conflicts), MIGRATION-FREE (head stays `0008`), 71 files / +4701−385.** Merge `b7fa0cfe`; `main` ff'd → `244ce773`. Reviewed with a 21-agent parallel risk analysis (5 dimensions, refute-by-default adversarial verify): **14 CONFIRMED / 2 OVERSTATED / 0 refuted, nothing medium or high.**

- **`#4447` `8145d66a` feat(memory): message processing** — the commit that needed the most scrutiny (deermem `queue.py` +147, `updater.py` +348, `manager.py` +46, **+7 lines to `config.example.yaml`**). ⚠ Our `config.yaml` is gitignored → new example keys are a SILENT-DEFAULT risk. **Resolved: of those 7 lines only ONE is a real key — `queue_max_depth: 1000` — and the CODE default is identically `1000`, so we inherit zero divergence** (`watermark_max_keys`, default 4096, is code-only and never written to the example file). **Consolidation untouched:** default still `False` (`deermem/config.py:178`) and BOTH gates (`updater.py:1045` prompt-build, `:1706` apply-time) are byte-identical — no hunk in the commit touches either line. Real changes (per-thread watermark that feeds only messages added since the last successful extraction; truncation changed head-1000 → head-500+tail-500; trivial-ack filter; 6-class signal patterns) land on the **extraction/write** path only. With `injection_enabled: false` those facts are never read back, so LGI output is unaffected and the net effect is *fewer, smaller* memory-extraction LLM calls competing with our qwen/kimi quota. ⚠ **Expect NEW log lines**: the host `extraction_callback` default IS injected for us (`backend_config: {}` does not opt out) → an INFO `Memory extraction metrics: …` per extraction and a WARNING when rejection rate >60%. **That WARNING is cosmetic for us — not an incident.** ⚠ **Do NOT set `patterns_dir`** — missing files there become a startup `FileNotFoundError`.
- **`#4460` `244ce773` linearize delta-mode checkpoint resume** — the delta-repair machinery is hard-gated on `accessor.mode != "delta"` and **we run `full`** (verified at runtime: `database.checkpoint_channel_mode = full`; code default `database_config.py:47-48`, key absent from our `config.yaml`). ⚠ **ONE change IS unconditional and executes on our full-mode path**: run start now wraps `_capture_rollback_point` in `async with _checkpoint_thread_lock(thread_id)` (`worker.py:660`) so a prior run still persisting duration metadata cannot interleave. Serialization/timing, not semantics; no re-entrancy so it cannot deadlock. ⚠ **Keep `checkpoint_channel_mode` OUT of `config.yaml`** — switching to `delta` is restart-required and effectively one-way.
- **`#4472` `bb9f67aa` close cancelled replacement admission** — **unreachable for us**: the cleanup only fires from a loop populated exclusively under `multitask_strategy in ("interrupt","rollback")`; ours is `"reject"` everywhere and the frontend sends no multitask field.
- **`#4365` `1c753124` terminal artifact delivery receipts (slice 1 of `#4272`)** — writes ONE extra row per **Gateway** run into the EXISTING `run_events` table (`run.delivery` / category `outputs`); no new table, column, migration, or config key. ⚠ **Hidden side effect worth knowing:** `RunJournal.run_inline = True` moves journal callbacks onto the run's event loop, which **activates two previously-dead paths** (incremental flushing every 20 events, run-progress DB writes every 5s) → steadier write traffic on UI runs. **Zero on the embedded path** — `RunJournal` is built only in `worker.py`, so `chat()` never creates one. "Slice 1" implies a follow-up that may need a migration later.
- **`#4442` pin recent chats** (pin flag rides the EXISTING `threads_meta.metadata_json` — no migration/endpoint/localStorage; **+3 additive i18n keys** `pinChat`/`unpinChat`/`pinChatFailed`), **`#4469` panel resize** (confined to `chat-box.tsx`, not a re-skinned file), **`#4471` dev-origins** — **DEFINITIVELY dev-server-only**: `allowedDevOrigins`' only consumer is Next's `blockCrossSite`, both call sites gated on the development bundler; our prod trust surface (`DEER_FLOW_TRUSTED_ORIGINS` → better-auth) is untouched.
- **`client.py` is UNCHANGED across all 7 commits → `chat()` and the LGI Stage-1 path are untouched.** No `RunStatus`/`stop_reason` changes (`runtime/runs/schemas.py` has a zero-byte diff across the range).
- **Carried patches all intact:** i18n **30/29** `WRI AI` (the 3 new keys inserted ~20 lines from our nearest rebrand line → clean auto-merge, as the dry run predicted); redis-strip 0 refs in compose AND `deploy.sh`; `UV_EXTRAS=postgres`; Dockerfile readabilipy+Playwright ×8; login `setupStatusPhase` graft ×6; nav-menu 0 deerflow refs.
- **Verified (all green):** `make up` exit 0, BOTH images rebuilt → frontend **`✓ Compiled successfully in 48s`** (the real TS gate proving all 3 new keys landed in `en-US.ts` + `zh-CN.ts` + `types.ts` — a missing key is a hard build failure, not a runtime fallback) → bootstrap `upgrade head (0008_thread_operation_kind)`, **0 boot errors**, **NO redis / NO provisioner**, `/` `/login` `/setup` all 200 → **3b `schema in sync`**, **`consolidation_enabled = False`**, `queue_max_depth = 1000` and `watermark_max_keys = 4096` both matching the expected defaults, `patterns_dir`/`prompts_dir` `None` → **3c Aio smoke** sandbox `7bac70d1` `SYNC726D_SANDBOX_OK / Linux / gem` → **`chat()` kimi-k3 / qwen3.7-plus / qwen3.7-max all `CHAT_OK`** → built bundle carries the 3 new keys (11 files) and 29 files with `WRI AI`; `/login` 17 WRI / 0 deerflow, `/setup` 10 WRI / 0 deerflow, landing 25 `WRI AI`. Rollback tag `pre-sync-20260726d` → `959fb30b` (drop post-push).
- ⚠ **BUNDLE-GREP GOTCHA (cost me a false negative):** the frontend build lives at **`/app/frontend/.next`**, NOT `/app/.next`. A grep against the wrong path returns `0` and *looks* like a clean pass. Always confirm the path resolves before trusting a zero.
- ✅ **THREE PRE-EXISTING BRANDING GAPS — FIXED in `ae42266a` (post-sync, separate commit; none were caused by this sync):** (1) `frontend/src/core/agents/api.ts:101` + `:113` — user-facing error **"Could not reach the DeerFlow backend."**; (2) `frontend/src/core/sidecar/context.ts:162` — sidecar system prompt says "the user's current DeerFlow chat"; (3) `en-US.ts:576` / `zh-CN.ts:553` — **"WRI AI AI bot"** double-AI from a `DeerFlow`→`WRI AI` substitution (⚠ the line numbers first reported, 573/550, were the *feishu* siblings — the defect is on the **`wecom`** key; grep the string, don't trust a remembered line number). (1) and (2) are **hardcoded TS strings outside the locale files**, which is exactly why the i18n rebrand never caught them — a locale-file-only rebrand audit is INCOMPLETE by construction; sweep `frontend/src/**/*.{ts,tsx}` too. Verified in the rebuilt bundle: 6 files carry each new string, 0 carry `WRI AI AI` or `DeerFlow backend`. **Deliberate retentions that must STAY:** `officialWebsite`, `githubTooltip`, `visitGithub`, and the Settings→About MIT credit — those correctly point at bytedance/deer-flow.
- 📌 **Deliberately NOT rebranded (assessed during the same sweep):** `components/landing/mobile-nav.tsx` and `components/landing/progressive-skills-animation.tsx` still say "DeerFlow" but have **zero importers** — dead upstream components our WRI landing re-skin replaced (`app/page.tsx` imports only Header/Hero/Capabilities/HowItWorks/Trust/CTA/Footer). And `frontend/src/content/**` (upstream's nextra docs, served at `/en/docs`, title "DeerFlow Documentation") carries **514 `DeerFlow` mentions across 72 of 88 files** — rebranding it would be a large permanent conflict surface for no gain. ⚠ **Context that makes both low-priority: the ENTIRE deployment is behind Caddy HTTP basic auth — `https://app.worldresearch.org/` and `/docs`, `/redoc`, `/openapi.json`, `/en/docs` ALL return 401 to the public.** Note `GATEWAY_ENABLE_DOCS` is unset → the FastAPI Swagger/ReDoc/OpenAPI endpoints are ENABLED (upstream's `backend/AGENTS.md` advises `false` in production); they are not publicly reachable today, so setting it false is optional defence-in-depth, not a live exposure.

Prior sync **2026-07-26 (later³)** — **1 commit (`e646188a`→`04b7e693`), DOCS-ONLY, fully INERT.** Merge `22b18847`; `main` ff'd → `04b7e693`. `#4397` fixes two links in `docs/superpowers/specs/2026-07-01-scheduled-tasks-mvp-design.md` that pointed at a contributor's absolute local worktree path (`/Users/nowcoder/Desktop/...`), replacing them with relative repo paths. **1 file, 2 lines, no code / config / migration / Dockerfile / compose.** `docs/` is excluded by our `.dockerignore` (line 50), so **neither image can change** — and we don't run the scheduler anyway. **Deliberately NO rebuild and NO 3a/3b/3c run:** the verification chain exists to catch behavioral regressions, and a markdown link fix outside the build context cannot produce one; the running stack was left up. Merge scope re-confirmed post-merge (`git diff 07c24db2..local-fixes` = that one doc file) and carried patches spot-checked intact (redis-strip 0 service refs, `UV_EXTRAS=postgres`, i18n 30/29 `WRI AI`, login `setupStatusPhase` graft ×6). No rollback tag cut — nothing to roll back.

Prior sync **2026-07-26 (later²)** — **2 commits (`dd3c5a17`→`e646188a`), CLEAN merge (0 conflicts), MIGRATION-FREE (head stays `0008`), backend-only, NO CVE.** Merge `7c563ff7`; `main` ff'd → `e646188a`. Zero carried-patch overlap (no compose / Dockerfile / i18n / login / setup / nav-menu / `config.example`); `git diff pre-sync..local-fixes -- frontend/` is **empty**, so the re-skin and the 49-string i18n rebrand were untouched by this merge.

- **`#4468` `e646188a` accept the SDK's default `stream_resumable=false`** — a **regression fix for `#4430`**, which *we merged 2026-07-24*. `#4430` tightened run-option validation, but `langgraph_sdk` drops only `None` from its payload, so `stream_resumable=False` reaches every request; the strict validator 422'd it. Upstream's own note says this **"422'd every IM channel run" (`#4466`)**. **We were never hit, for two independent reasons:** (1) our frontend's `core/api/stream-mode.ts` shim *deletes* `streamResumable` before sending (it is set `true` in `threads/hooks.ts` and stripped with a warning), so the field never reached the validator; (2) **we run no IM channels**. Taken anyway — it closes the exposure for any raw-SDK client. `run_models.py` now accepts `Literal[False] | None` and rejects only an explicit `True`.
- **`#4309` `2f60bee3` surface length-capped model responses** — new `ModelLengthFinishReasonMiddleware` + `model_length_termination_detectors.py`, added **unconditionally** to the lead chain (NOT config-gated) → live code on our run path, so it was read line-by-line rather than trusted: **every `_apply` path returns `None`**, it never returns a state update, never mutates messages, and never reparses text into tool calls. Its only effect is stamping `ctx["stop_reason"] = "model_length_capped"` (only if unset) + an INFO log. **`worker.py:772` still calls `set_status(run_id, RunStatus.success, stop_reason=...)` — a length cap does NOT flip the run to failed**, the reason rides alongside success. Detectors: `openai_compatible_length` (`finish_reason == "length"` — **covers our qwen/kimi**), `anthropic_max_tokens`, `gemini_max_tokens`. **`client.py` UNCHANGED and does not read `stop_reason` at all → `chat()` and the LGI Stage-1 path are behaviourally identical.** ⚙ **Genuinely useful to us:** all our active models run `max_tokens: 32768` (one at 8192) and LGI Stage-1 parses JSON out of long qwen responses — a length-capped response previously surfaced only as a downstream JSON parse error with no explanation; now it leaves a `Provider model length cap detected` log line naming the detector and run.
- ⚠ **CONFIG-SHAPE NOTE (not from this sync, found during 3b): `MemoryConfig.consolidation_enabled` NO LONGER EXISTS.** The 2026-07-15 memory restructure abstracted the backend, moving every DeerMem-private field into `memory.backend_config` (`config/memory_config.py::_LEGACY_DEERMEM_FIELDS` auto-migrates old top-level keys on load). **The old probe `MemoryConfig().consolidation_enabled` now raises `AttributeError` — it is no longer a valid check.** The correct probe is `DeerMemConfig(**(cfg.memory.backend_config or {})).consolidation_enabled`. Verified this sync: our `config.yaml` has `backend_config: {}`, the DeerMem default is still `False`, and the **effective runtime value is `False`**. Consolidation remains OFF.
- **Verified (all green):** `make up` exit 0 (gateway image rebuilt, Playwright/readabilipy layer CACHED → Dockerfile patch intact) → bootstrap `branch=versioned -> upgrade head (0008_thread_operation_kind)` (no new migration) → **0 boot errors/tracebacks**, **NO redis / NO provisioner**, `:2026` 200 → **3b `schema in sync — no missing columns`** + **effective `consolidation_enabled=False`** + `stream_bridge = None` (memory backend — validates the redis-strip) → **`chat()` kimi-k3 / qwen3.7-plus / qwen3.7-max all `CHAT_OK`** → **3c Aio smoke** sandbox `1b905e9b` `SYNC726C_SANDBOX_OK / Linux / gem` → **new middleware confirmed in the LIVE chain** (25 middlewares; neighbours `TerminalResponseMiddleware` → `ModelLengthFinishReasonMiddleware` → `SafetyFinishReasonMiddleware`, matching upstream's documented order) → **`/login` 200 (17 WRI refs, 0 deerflow), `/setup` 200 (10 WRI refs, 0 deerflow)**, landing bundle carries 25 `WRI AI` strings. Rollback tag `pre-sync-20260726c` → `8377d61f` (drop post-push).
...
⚙ **FEISHU/LARK CLI — decision: KEEP, do not strip.** Feishu (飞书) is ByteDance's Slack/Teams competitor; "Lark" is the same product's international brand, and deer-flow (a ByteDance project) ships a managed Lark integration whose CLI `#3971` now bakes into `backend/Dockerfile`. **We do not use it** (no IM channels configured; our deer-flow serves `DeerFlowClient.chat()` for LGI Stage-1 + the outreach discovery worker). Kept anyway because stripping it would add a THIRD carried patch to a Dockerfile upstream actively edits — a permanent conflict surface — to save **41 MB of a 3.31 GB image (~1.2%)**; for scale our own Playwright/chromium patch is **656 MB**. It never runs at runtime unless the integration is configured. ⚠ **It IS a new build-time network dependency**: `npm install -g @larksuite/cli@1.0.65 && lark-cli --version` — if that ever fails, the image build fails; remember this when diagnosing a broken rebuild. **NO impact on Playwright — verified in the running image: `lark-cli 1.0.65`, `playwright 1.62.0`, `chromium-1234`, `@playwright/mcp` all present.** The one plausible interaction is that the Lark step runs `npm config set registry` **when `NPM_REGISTRY` is set**, which persists to later layers and would redirect our Playwright install too — but `NPM_REGISTRY` is unset in our build, so it is a no-op. Only relevant if a registry mirror is ever introduced.

Prior upstream sync: **2026-07-26** — **14 commits (`b41a8d44`→`009b4ffc`), ⚠ 3 CONFLICTS (first conflicted sync since 07-19), MIGRATION-FREE (head stays `0008`), NO CVE.** Merge `ffddc017`; `main` ff'd → `009b4ffc`.
- **⚠ THE 3 CONFLICTS were all OUR WRI RE-SKIN vs REAL upstream logic — resolved keep-OURS-skin + GRAFT-upstream-logic (never a blind "keep ours"):**
  1. **`(auth)/login/page.tsx`** ← `#4371`. Grafted: `setupStatusChecked: boolean` → **`setupStatusPhase: "checking"|"ready"|"unavailable"` + `setupStatusAttempt`**; the registration gate becomes `checked: setupStatusPhase === "ready"` — a **security-relevant fix** ("a failed probe must not expose registration while the system's setup state is unknown"); the effect is **SPLIT** so retrying a slow Gateway no longer refetches SSO provider config (`}, [setupStatusAttempt]` + a separate SSO effect). Added upstream's retry banner **restyled to the WRI skin** (amber, to stay distinct from our maroon setup notice).
  2. **`(auth)/setup/page.tsx`** ← `#4371`. Added `"unavailable"` to `SetupMode`, `useI18n`, a retry counter, `setMode("loading")` on (re)probe, and **`.catch` → `setMode("unavailable")` instead of `router.replace("/login")`** — a slow Gateway used to bounce silently to login, hiding a real outage. New panel rendered in OUR `PageShell` + `BrandHeader`.
  3. **`workspace/workspace-nav-menu.tsx`** ⚠ **THIS FILE IS PART OF OUR CARRIED RE-SKIN AND WAS NOT PREVIOUSLY DOCUMENTED** — we de-brand the nav menu (remove deerflow.tech / GitHub / issues / mailto links, the About item, and the Bug/Globe/Info/Mail/Github icons + a `DropdownMenuSeparator`). Upstream refactored `SettingsDialog` local state → the **`useSettingsDialog()` hook** (`openSettings(section)`). Resolved by starting from THEIRS (mechanical refactor) and re-applying our deletions; note **the About item carried upstream's `openSettings("about")` call site**, so only `openSettings("appearance")` remains.
- **⚠ `#3971` Lark CLI is now installed into OUR production image** — `backend/Dockerfile` gained a global `npm install -g @larksuite/cli@1.0.65` + `lark-cli --version` build step (auto-merged; **our readabilipy + Playwright patch survived, verified**). It is a **new build-time network dependency for a Feishu/Lark feature we do not use** — if that npm install ever fails, our image build fails. Watch for it on a future rebuild.
- **`client.py` UNCHANGED. NO consolidation commit → OFF. `config_version` unchanged. Migration-free (head stays `0008`).**
- **Rest positive/inert:** frontend perf (`#4425` coalesce streaming renders to a frame budget, `#4441` stop re-deriving message content per chunk, `#4411` index tool-call results), `#4101` case-insensitive email accounts, `#5d073991` widen boxlite/aio tenant hash + verify identity on reclaim, `#4361` summarize with the run model + fall back on summary-provider failure, `#4381` unwrap Overwrite-wrapped sandbox state, `#4459` scope branch-history seed run ids, `#4367` browser-extra detection for indentless YAML, `#4453` frontend hook tests in a DOM env, postcss 8.5.6→8.5.18.
- **Verified (all green):** frontend Docker build **`✓ Compiled successfully`** (the real typecheck for the 3 resolutions — `pnpm` is not on the host) → bootstrap `upgrade head (0008_thread_operation_kind)` (no new migration) → 0 boot errors, NO redis/provisioner, `:2026` 200 → **3b `schema in sync`** + consolidation OFF → **`chat()` kimi-k3/qwen3.7-plus/qwen3.7-max all `CHAT_OK`** → **3c Aio smoke** sandbox `9abb4953` `SYNC726_SANDBOX_OK` → **`/login` + `/setup` both HTTP 200, WRI-branded, 0 deerflow refs**. Rollback tag `pre-sync-20260726` → `0ea4c4bb` (drop post-push).

Prior upstream sync: **2026-07-25** — **9 commits (`58befaf2`→`735f67a5`), CLEAN merge (0 conflicts), NO CVE. head `0007`→`0008`.** Merge `6388dcf5`; `main` ff'd → `735f67a5`. Scope: 60 files, +5021/−374. **Runtime/checkpoint robustness; nothing touches our operational path.**
- **⚠ MIGRATION `0008_thread_operation_kind` — additive, safe.** `down_revision="0007_scheduled_run_active_index"`; `upgrade()` is a single `safe_add_column` on `runs`: `operation_kind VARCHAR(32) NOT NULL DEFAULT 'run'` — the server default backfills existing rows, so no table rewrite and no data loss (`drop_column` is downgrade-only). **VERIFIED at the DB:** bootstrap logged `Running upgrade 0007_scheduled_run_active_index -> 0008_thread_operation_kind`, `alembic_version` = `0008_thread_operation_kind`, and `runs.operation_kind` exists with default `'run'::character varying`. ⚠ **code rollback now needs `alembic downgrade 0007`.**
- **⚠ `client.py` touched by `#4422` `0f0955bf` fix(client): preserve ToolMessage artifacts — but NOT `chat()`.** The change is in STREAM-event serialisation (`StreamEvent(type="messages-tuple")`), adding an optional `"artifact"` key to tool-result events when the source `ToolMessage` carries one. Purely additive, on the `stream()` path. **`chat()` VERIFIED kimi-k3 + qwen3.7-plus + qwen3.7-max, all `CHAT_OK`.**
- **Robustness (inert-positive):** `#4421` linearise checkpoint message-write merging, `#4437` serialise checkpoint writes with active runs, `#4427` lease-aware orphan reconciliation (follow-up to the `#4434` work already taken), `#4450` guard pending-run startup cancellation, `#4432` keep completed subtask cards stable after reload. **Inert:** `#4391` E2B replica capacity (we use Aio), `#4456` MCP malformed path-like text, `#4392` Telegram inbound attachments (no Telegram).
- **NO consolidation commit → OFF. `config_version` unchanged. `#4174` tool-schema fix intact.** Carried patches intact — ⚠ upstream made LARGE edits to `frontend/src/core/i18n/locales/en-US.ts` (runDuration/browser/voiceInput blocks) and the **`AI can make mistakes` disclaimer re-skin SURVIVED the merge** (verified). **Verified (all green):** rebuild → bootstrap `upgrade head (0008_thread_operation_kind)` → 0 boot errors, NO redis/provisioner, `:2026` 200 → **3b `schema in sync`** + consolidation OFF → **`chat()` ×3 `CHAT_OK`** → **3c Aio smoke** sandbox `8e6fbbbe` `SYNC725_SANDBOX_OK`. Rollback tag `pre-sync-20260725` → `ecacfc83` (⚠ also needs `alembic downgrade 0007`; drop tag post-push).

Prior upstream sync: **2026-07-24 (later)** — **7 commits (`159b7749`→`16b612cf`), CLEAN merge (0 conflicts), NO CVE. head `0006`→`0007`.** Merge `d2096f1b`; `main` ff'd → `16b612cf`. **LOW-RISK — one additive index migration + robustness fixes; nothing touches our operational path.**
- **⚠ MIGRATION `0007_scheduled_run_active_index` (`#4105` scheduler dup-dispatch fix) — additive + inert for us.** `down_revision="0006_agents"`; adds a PARTIAL UNIQUE index on `scheduled_task_runs` ("only one ACTIVE scheduled run") with a dedupe-then-index step. The table exists (created by `0003_scheduled_tasks`); **we don't run the scheduler** (no `scheduler` block in `config.yaml` → disabled), so the table is empty → dedupe is a no-op and the unique index creates trivially. **The whole scheduler feature (`#4105` + `app/scheduler/service.py`) is INERT for us.** Applied CLEANLY by bootstrap (`Running upgrade 0006_agents -> 0007_scheduled_run_active_index`), `schema_sync` green after. ⚠ **code rollback now needs `alembic downgrade 0006`.**
- **INERT:** `#4167` skills non-string frontmatter keys, `#4081` subagent-limit clamp (we use `subagent_enabled=False`), `#3563` skills FS-IO offload, `#4440` docs (subagent runaway guards), `#4133` GitHub-channel follow-up buffering (no GitHub channel), `#4446` frontend image maxWidth.
- **`client.py` UNCHANGED → chat intact. NO consolidation commit → OFF. `config_version` unchanged.** Carried patches intact (incl. disclaimer re-skin). **Verified (all green):** rebuild → bootstrap `upgrade head (0007_scheduled_run_active_index)` (0006→0007 applied) → 0 boot errors, NO redis/provisioner, `:2026` 200 → **3b `schema in sync`** + consolidation OFF → **`chat()` kimi-k3/qwen3.7-plus/qwen3.7-max all `CHAT_OK`** → **3c Aio smoke** sandbox `6e641296` `SYNC724B_SANDBOX_OK`. Rollback tag `pre-sync-20260724b` → `fe3158cd` (⚠ code rollback also needs `alembic downgrade 0006`; drop tag post-push).

Prior upstream sync: **2026-07-24** — **6 commits (`959f0527`→`c7538cfb`), CLEAN merge (0 conflicts), MIGRATION-FREE (head stays `0006`), NO CVE.** Merge `15d8d6d0`; `main` ff'd → `c7538cfb`. **LOW-RISK — all robustness/CI fixes, nothing touches our operational path.**
- **`#4434` `80c06414` lease-aware orphan reconciliation** + **`#4420` `c7538cfb` terminate orphaned streams after lease recovery** — sandbox/stream cleanup robustness (relates to the `#4221`/`#4206` ownership work; we're single-instance memory-ownership → inert-positive; 3c Aio smoke unaffected).
- **`#4430` `a4ede80d` reject unsupported run options / stream modes** — the one worth a chat recheck (validates run-option/stream-mode enums). Our `chat()` uses only supported options (`model_name`/`thinking_enabled`/`subagent_enabled`/`recursion_limit`) → **VERIFIED `chat()` kimi-k3 + qwen3.7-plus + qwen3.7-max all `CHAT_OK`**.
- **INERT:** `#4358` preserve regenerate state in branched threads; `#4438` GIF support in `view_image`; `#4435` lock ruff version (CI-only).
- **`client.py` UNCHANGED → chat intact. NO consolidation commit → OFF. `config_version` unchanged.** Carried patches intact (incl. the `AI can make mistakes` disclaimer re-skin). **Verified (all green):** rebuild → bootstrap `upgrade head (0006_agents)` (no new migration) → 0 boot errors, NO redis/provisioner, `:2026` 200 → **3b `schema in sync`** + consolidation OFF → **`chat()` all 3 models `CHAT_OK`** → **3c Aio smoke** sandbox `d9a8ef54` `SYNC20260724_SANDBOX_OK`. Rollback tag `pre-sync-20260724` → `be0139fa` (migration-free — plain `git reset`; drop post-push).

Prior upstream sync: **2026-07-23 (later)** — **12 commits (`70fb9165`→`5f0108f5`), CLEAN merge (0 conflicts), MIGRATION-FREE (head stays `0006`), NO CVE.** Merge `4463714f`; `main` ff'd → `5f0108f5`. **Mostly frontend/gateway/streaming fixes; one auth feature, INERT-by-default.**
- **`#4370` `7857fa0c` feat(authz): enforce tool authorization at assembly and runtime — OFF BY DEFAULT** ("authorization is disabled by default"). Adds `authz/` (tool_filter/enforcement/adapter/rbac) that filters denied tools before the model + re-checks before each business-tool execution — BUT all gated on `authorization.enabled` (we have NO `authorization` block in `config.yaml` → defaults False), and `filter_visible_tools` is a no-op when `provider is None`. **⚠ this touched `client.py`** — authz-identity plumbing inside `_ensure_agent` (`if self._app_config.authorization.enabled:` … `build_principal_from_context`; a new `*, context=None` kwarg) + a tool-assembly reordering that RESTORES legacy ordering when authz is off. **`chat()` PUBLIC API unchanged → VERIFIED `chat()` works kimi-k3 + qwen3.7-plus + qwen3.7-max (all `CHAT_OK`)** with the full toolset — the tool-assembly change is safe with authz off. (`DeerFlowClient.stream()` treats its in-process caller as trusted, accepts identity kwargs — the LGI pipeline uses `chat()`, unaffected.)
- **`#4394` `b7933d18` fix(safety): backfill empty content-filter responses so they don't poison the thread** — robustness (inert-positive for the research path).
- **INERT:** gateway branch-fork run-events (`#4385`), webhook 200-delivery (`#4307`), run-event stream contract (`#4342`); frontend run-duration display, orphan tool-message visibility, `<current_uploads>` tag parse; streaming custom-events (`#4403`) + subgraph-frame fix (`#4407`); a new `first-principles` skill workflow (`#4398`); a checkpoint bench (`#4395`).
- **NO consolidation commit → consolidation VERIFIED OFF.** `config.example.yaml` config_version UNCHANGED (29→? no change). `#4174` list_uploaded_files fix still present (chat toolset binds). Carried patches intact. **Verified (all green):** rebuild → bootstrap `upgrade head (0006_agents)` (NO new migration, head stays 0006) → 0 boot errors, NO redis/provisioner, `:2026` 200 → **3b `schema in sync`** + consolidation OFF + 6 models (incl **kimi-k3**) → **3c Aio smoke** sandbox `59a6f1e6` `SYNC20260723B_SANDBOX_OK` → **`chat()` kimi-k3/qwen3.7-plus/qwen3.7-max all `CHAT_OK`**. Rollback tag `pre-sync-20260723b` → `36f5d72b` (migration-free — plain `git reset` rollback, NO `alembic downgrade` needed; drop tag post-push).

Prior upstream sync: **2026-07-23** — **24 commits (`cd862756`→`a38b1dae`), CLEAN merge (0 conflicts), NO CVE. ⚠ FIRST SCHEMA CHANGE SINCE 0005 → head `0005`→`0006`.** Merge `1a1aca80`; `main` ff'd → `a38b1dae`. **This is the batch that was BLOCKED 2026-07-22 by `#4174` and rolled back — NOW UNBLOCKED.**
- **✅ `#4174` blocker FIXED by `#4375` `7b330101` fix(tools): exclude injected runtime from list_uploaded_files schema.** The bug was `runtime: Annotated[Runtime, InjectedToolArg] | None = None` (InjectedToolArg nested in the `| None` union → LangChain didn't strip it → the `Runtime` callable leaked into the tool JSON schema → `bind_tools` failed for EVERY `chat()`). Fix = plain `runtime: Runtime` (LangChain auto-injects `Runtime`-typed params, excluding them from the schema). **VERIFIED — `chat()` works across `kimi-k3` + `qwen3.7-plus` + `qwen3.7-max` (all returned OK), full lead-agent toolset binds.**
- **⚠ MIGRATION `0006_agents` (`#4359` `0d4d0cb1` db-backed custom-agent storage) — first alembic revision since 0005.** ADDITIVE: `down_revision="0005_run_stop_reason"`, `upgrade()` only `create_table("agents")` + `ix_agents_user_id` index (drop is downgrade-only). Applied CLEANLY by bootstrap (`Running upgrade 0005_run_stop_reason -> 0006_agents`), `schema_sync` green after. **UNUSED for us — `agent_storage.backend` defaults to `file`** (db opt-in; startup validation only fires if you set `backend: db`), so the `agents` table is created but empty. ⚠ **ROLLBACK NOW NEEDS `alembic downgrade 0005` first** (DB at 0006 would be ahead of pre-0006 code).
- **⚠ `#4279` `4bf028d0` memory v1→v2 Markdown storage migration** present but LAZY (first-read) — did NOT trigger (we run `injection_enabled: false`); our `.deer-flow` memory UNTOUCHED (0 `.v1.bak`, `memory.json` intact). Snapshot at `~/deerflow-memory-premigration-20260723.tgz` (+ scratchpad copy), 4.1MB — precaution per upstream ("one-way; snapshot storage root before upgrading").
- **`client.py` changed** — checkpoint-mode plumbing (`freeze/inject_checkpoint_mode` from `#4292` `42baed8c`) + a middleware-build arg refactor. **`chat()` public API unchanged → LGI pipeline unaffected (empirically verified above).**
- **INERT/opt-in:** `#4347` per-agent model settings (agent-config layer), `#4292` dual-mode checkpoint (default `full` = current behavior), `#4326` pluggable MemoryManager (default stays DeerMem), E2B sandbox fixes (we use Aio), `#4240` mcp OAuth threading lock, `#4267` transient-image cleanup, frontend/TUI/Feishu/docs/AI-disclaimer. **NO consolidation commit → consolidation VERIFIED OFF** (`consolidation_enabled=False`, manager=DeerMem). **`config.example.yaml` config_version stays 29.** Carried patches intact (Dockerfile/`.dockerignore`/`UV_EXTRAS`/WRI re-skin/kimi-k3). **Verified (all green):** rebuild → bootstrap `upgrade head (0006_agents)` (0005→0006 applied) → 0 boot errors, NO redis/provisioner, `:2026` 200 → **3b `schema in sync`** + consolidation OFF + DeerMem → **3c Aio smoke** sandbox `13976566` `SYNC20260723_SANDBOX_OK` → **`chat()` kimi-k3/qwen3.7-plus/qwen3.7-max all OK**. Rollback tag `pre-sync-20260723` → `26143719` (⚠ code rollback also needs `alembic downgrade 0005`; drop tag post-push).

Prior (blocked/rolled-back) attempt: **2026-07-22** — attempted the same batch (then 17 commits, `3c0a45ad`→`8c78d1f4`) but ROLLED BACK: `#4174` `list_uploaded_files` tool leaked its injected `Runtime` into the JSON schema → `bind_tools` broke `chat()`. Restored `local-fixes` to `26143719`, verified `KIMI_K3_OK`; memory never touched. Resolved above by the `#4375` fix.

Prior upstream sync: **2026-07-21** — **10 commits (`97ca7f88`→`3c0a45ad`), CLEAN merge (0 conflicts), MIGRATION-FREE (head stays `0005`), NO CVE.** Merge `e59af04e`; `main` ff'd → `3c0a45ad`. **LOW-RISK batch — nothing touches our operational path (Aio sandbox / `chat()` / memory / DB schema all unaffected).**
- **`#4187` `fa496c0c` feat(browser): agentic browser control — OPT-IN, OFF BY DEFAULT.** A Playwright `navigate→observe→click/type` tool group (`browser_navigate`/`browser_snapshot`/`browser_click`/…, `group: browser`) shipped as an OPTIONAL `uv --extra browser`; you must UNCOMMENT the browser tools in `config.yaml` AND install the extra to use it. We don't → no Playwright dep, no startup change (Gateway only fails-fast if browser control IS configured but Playwright missing; `/api/features` `browser_control.enabled` gates the UI). ✓ verified `browser_navigate` NOT in our `config.yaml`, startup clean. **A genuinely useful LATENT capability for the [[lgi_pipeline]] research path** (stateful, SSRF-screened, `[ref]`-addressed snapshots — more than the read-only `web_fetch`/`web_capture`); ⚠ requires `GATEWAY_WORKERS=1` if ever enabled (process-local browser sessions).
- **`#4325` `da3feb38` + `#4329` `d2116d86` sandbox fixes — E2B ONLY** (`community/e2b_sandbox/*`); we run the **Aio** sandbox → **inert** (3c smoke verified `AioSandboxProvider` sandbox `34434a95` unaffected).
- **`#4117` `97ca7f88` frontend artifact/markdown rendering hardening** (inert-positive). **Dep bumps:** pillow `12.2.0→12.3.0` (routine, likely a security patch), mcp `1.27.0→1.28.1` (minor, `uv.lock` only — rebuild clean). **Inert:** `#4303` workspace-changes diff-line counting, `#4337` /goal length validation, `#4335` Feishu send_file guard (no Feishu), `#4321` Langfuse skill-scan metadata (no Langfuse).
- **`client.py` UNCHANGED → `chat()` intact → LGI pipeline unaffected. NO memory/consolidation commit → consolidation VERIFIED OFF** (`consolidation_enabled=False`, `prompts_dir`=None). **`config.example.yaml` config_version stays 29.** **Carried patches intact** (Dockerfile readabilipy+Playwright ✓ — the browser feature did NOT modify the base Dockerfile since it's an optional extra; `.dockerignore` ✓; `UV_EXTRAS=postgres` ✓; WRI login/setup re-skin ✓). **Verified (all green):** `make down && make up` → bootstrap `upgrade head (0005_run_stop_reason)` (NO new migration) → 0 boot errors (no Playwright/browser errors), NO redis/provisioner, `:2026` 200 → **3b "schema in sync"** + **6 models** (incl **kimi-k3**) → **consolidation OFF** + **browser NOT enabled** → **3c Aio smoke** sandbox `34434a95` / `SYNC20260721_SANDBOX_OK / Linux / python3.10` → `DeerFlowClient.chat(model_name="kimi-k3")` → `KIMI_K3_OK` (clean). Rollback tag `pre-sync-20260721` → `88a2ac36` (drop post-push).

Prior upstream sync: **2026-07-20 (later²)** — **16 commits (`09e25b8a`→`b565e6c0`, i.e. everything after `1a1c5def`), CLEAN merge (0 conflicts), MIGRATION-FREE (head stays `0005`), NO CVE.** Merge `54c9a2b6`; `main` ff'd → `b565e6c0`. Scope: 43 files, +7563/−143 (mostly tests). **A bigger, feature-heavy batch — every new-behavior feature is INERT-BY-DEFAULT and verified from the diffs.** Highlights:
- **`#4294` `ac5fd462` feat(backend): bound LLM concurrency + shed burst-rate (429) retries.** ⚠ **NUANCE relevant to the LGI Stage-1 qwen primary path — my first read was WRONG and an adversarial verify corrected it:** the burst-shed tight budget is NOT gated on the (default-OFF, `max_concurrent_calls: 0`) internal limiter — it's assigned by **pattern-matching the PROVIDER's 429 text/code** against `_BURST_PATTERNS = {limit_burst_rate, "rate increased too quickly", "burst rate", 请求速率增长过快, 突发速率}`. Those Chinese phrases are **Alibaba Bailian/Dashscope** burst signals, and Bailian is the **LGI Stage-1 qwen3.7-plus/max provider**, so a qwen Bailian burst-rate 429 now gets a **tightened retry budget (3→2 attempts, +5000ms backoff) then sheds** — fail-FASTER to cross-family escalation (defensible: retrying into a burst-throttle worsens the slope; the orchestrator has escalation fallback). **A kimi/Moonshot/OpenRouter upstream 429 ("rate limit exceeded" + `Retry-After`) does NOT match `_BURST_PATTERNS` → stays `transient` → full `retry_max_attempts` budget → recovery preserved** (empirically `KIMI_K3_OK` this sync). The limiter wraps a single attempt (not the retry loop) and is off by default anyway.
- **`#4230` `39493406` harden Postgres async engine (`pool_recycle` + `command_timeout`) — BENEFICIAL to us:** kills stale-connection 504s on our managed-PG backend. `persistence/engine.py` + `database_config.py`, defaults, no required key.
- **`#4221` `5eb59cb1` + `#4206` sandbox orphan-reconcile peer-kill fix — VALIDATES our redis-strip.** The refactor adds an explicit startup message: *"Sandbox ownership store cannot coordinate across processes (`sandbox.ownership.type: memory`). Safe for a SINGLE gateway instance only — multi-worker/load-balanced … must set `sandbox.ownership.type: redis`, or peers … idle-destroy each other's live sandboxes."* **That is exactly us** — the in-memory ownership backend is a first-class single-instance mode; redis is only needed for multi-worker. Our redis-strip carried patch stands (touches `ownership/redis.py` +214 and `ownership/memory.py`, merged clean, our non-redis path smoke-verified).
- **`#4260` `92c8f2f0` feat(authz): RBAC provider + factory — OFF BY DEFAULT** (`AuthorizationConfig(enabled=False)` → `if not config.enabled: return None`; no import). Inert unless explicitly enabled.
- **`#3964` `ca16b64b` feat(agent): config-declared lead middlewares — OPT-IN** (`extensions.middlewares` in config; we don't set it). ⚠ **This is the commit that touched `client.py` — but ONLY the MCP/skills config-WRITE helpers (`config_data = current_config.to_file_dict()` so the new field survives a write), NOT `chat()`.** `DeerFlowClient.chat()` (LGI pipeline hot path, `stage1_country_research.py:260,292`) is byte-for-byte unchanged → **pipeline unaffected.**
- **3 memory commits — consolidation VERIFIED OFF:** `#4216` `cd0f1e22` (memory config-reload) was **REVERTED in-batch by `#4320` `1073393e`** = net-zero; `#4225` `8511fa6a` consolidated-facts inherit `expected_valid_days` (behavior INSIDE consolidation, only active if ON); `#4208` `9eebc6a9` `_memory_config`/AppConfig auto-reload sync (config plumbing). **No default flip** — re-verified `consolidation_enabled=False`, `prompts_dir`/`patterns_dir`=None.
- **Inert:** `#4057`/`#4315` SkillScan security hardening (AST shell=True bypass, PEP 695 alias), `#4319` serper malformed-URL guard, `#4306` TUI `clear`, `#4197` Slack mrkdwn escape (no Slack), `#4170` workspace-changes symlink-vs-delete classification.
**`config.example.yaml` config_version 28→29** (new OPTIONAL sections: authz/sandbox/database/`llm_call`); **our instance `config.yaml` stays v9 and is unaffected** (as always — the instance version doesn't track the template; no new REQUIRED key blocks startup, empirically confirmed by bootstrap). **Carried patches intact:** Dockerfile readabilipy+Playwright ✓, `.dockerignore` ✓, `UV_EXTRAS=postgres` ✓, WRI login/setup re-skin (maroon `#7b1e2b` + remember-me) ✓. **Verified (all green):** `make down && make up` → bootstrap `branch=versioned → upgrade head (0005_run_stop_reason)` (NO new migration) → 0 boot errors, NO redis/provisioner, `:2026` 200 → **3b "schema in sync — no missing columns"** + **6 models** (incl **kimi-k3**) → **consolidation VERIFIED OFF** → **3c Aio bash smoke** sandbox `b6e4d6bb` / `SYNC20260720C_SANDBOX_OK / Linux / python3.10` → `DeerFlowClient.chat(model_name="kimi-k3")` → `KIMI_K3_OK` (clean, no 429 this run). Rollback tag `pre-sync-20260720c` → `138f8969` (drop post-push).

Prior upstream sync: **2026-07-20 (later)** — **3 commits (`16377b12`→`1a1c5def`), CLEAN merge (0 conflicts), MIGRATION-FREE (head stays `0005`), NO CVE.** Merge `0106c99f`; `main` ff'd → `1a1c5def`. Scope: 6 files, +617/−1 (mostly tests). **⚠ HEADLINE — `#4314` `1a1c5def` fix(agents): classify web_fetch ERROR PAGES as errors, not successful evidence — GENUINELY RELEVANT to the LGI research path.** Previously a fetched 404/5xx/gateway **error shell** could be handed to the model as successfully-retrieved page content, i.e. Stage-1 research could cite an error page as if it were a real source. Adds `agents/middlewares/tool_result_meta.py` (+365-line fixture test) that classifies error shells by status and attaches category metadata — 500/501 → *stop*, gateway/5xx → *try_alternative* (a different source), keeping the classification identical to `_classify_error_text`. Gate is `_PAGE_CONTENT_TOOL_NAMES = {"web_fetch"}` (all first-party fetch providers normalise to `web_fetch`, so it's provider-agnostic); `web_capture` is deliberately excluded (its dead-target signal is the provider warning suffix). **Complements the [[lgi_pipeline]] Stage-2 verifier: Stage-2 catches bad citations AFTER the fact; this reduces them AT SOURCE.** **The other 2 are INERT:** `#4312` `16377b12` frontend encode uploaded filenames in delete requests; `#4316` `b02308bb` DingTalk drop inbound messages with no conversation identity (no IM channels configured). **Memory subsystem UNTOUCHED — consolidation STILL OFF** (no memory commit in range; re-verified `consolidation_enabled=False`, `prompts_dir`/`patterns_dir`=None). **`config.example.yaml` UNTOUCHED** (config_version stays 28). **`client.py` UNCHANGED → `DeerFlowClient.chat()` intact → LGI pipeline unaffected.** **Carried patches intact:** Dockerfile readabilipy+Playwright ✓, `.dockerignore` `.deer-flow` ✓, `UV_EXTRAS=postgres` ✓, WRI re-skin (login/setup incl. remember-me graft) ✓. **Verified (all green):** `make down && make up` → bootstrap `branch=versioned → upgrade head (0005_run_stop_reason)` (NO new migration) → "Application startup complete", 0 boot errors, NO redis/provisioner, `:2026` 200 → **3b sentinel "schema in sync — no missing columns"** + **6 models** (deepseek-v4-pro/**kimi-k3**/qwen3.6-plus/qwen3.7-plus/qwen3.7-max/mimo-v2.5-pro) → **consolidation VERIFIED OFF** → **3c Aio bash smoke** sandbox `b05db291` / `SYNC20260720B_SANDBOX_OK / Linux / python3.10 / uv0.8.9` → **BONUS** `DeerFlowClient.chat(model_name="kimi-k3")` → `KIMI_K3_OK`. ⚠ **OBSERVED (benign): kimi-k3 returned a transient upstream 429** ("moonshotai/kimi-k3 is temporarily rate-limited upstream", provider Moonshot AI, `Retry-After: 1`) on attempts 1–2; **the harness retry recovered on attempt 3** — so OpenRouter's free-tier kimi can rate-limit under load; the LGI cross-family escalation should tolerate a retry (it does). Rollback tag `pre-sync-20260720b` → `d20d6651` (drop post-push).

Prior 2026-07-20 — **12 commits (`5b65d543`→`cd34a1a5`), CLEAN merge (0 conflicts — login/setup UNTOUCHED), MIGRATION-FREE (head stays `0005`), NO CVE.** Merge `b1c7605b`; `main` ff'd → `cd34a1a5`. Scope: 59 files, +4236/−480. **⚠ HEADLINE — `#4286` `de024646` feat(memory): template externalization** (the only memory-subsystem commit): a REFACTOR that moves the DeerMem prompts/patterns to YAML (`core/prompts/*.yaml`, `core/message_patterns/*.yaml`) and adds two OPTIONAL `DeerMemConfig` fields `prompts_dir`/`patterns_dir` (default `None` = bundled = pre-externalization behavior). **It does NOT touch `consolidation_enabled`** → consolidation stays OFF; **rigorously re-verified under the new code** (see 3b): `DeerMemConfig.from_backend_config({}).consolidation_enabled=False`, field default `False`, and `prompts_dir`/`patterns_dir` both resolve to `None` (bundled). **Security hardening:** `#4236` `0cd55067` reject colon in zip member names (NTFS ADS smuggling on skill install) + `#4265` `a8bf54cb` skillscan detects exfil through instance/dataflow network clients (both matter for the untrusted-content/skill path). **Small robustness for the research path:** `#4239` `d075be02` surface web_fetch target-page error status (the LGI research web_fetch tool), `#4301` `a9a57fb7` handle null `config.configurable` in gateway `_resolve_thread_id`. **Config robustness:** `#4275` `3ed2e1f1` #4124 follow-ups (shared signature helper `config/file_signature.py`, `resolve_config_path` None contract). **Frontend (inert for the pipeline):** `#4282` `39cc26dd` per-thread composer drafts, `#4302` encode thread IDs. **Inert:** `#4287`/`#4274` channel dedupe (no IM channels), `#4289` webhook doc, `#4252` skills tracing. **⚠ `client.py` NOT changed → `DeerFlowClient.chat()` interface intact → LGI pipeline unaffected.** **`config.example.yaml`** = `config_version 27→28` + two COMMENTED optional memory keys (`prompts_dir`/`patterns_dir`, default `None` = bundled) → inert; instance `config.yaml` governs; the bump caused NO startup warning/block (bootstrap clean). **Carried patches intact:** Dockerfile readabilipy+Playwright ✓, `.dockerignore` `.deer-flow` ✓, `UV_EXTRAS=postgres` ✓, WRI re-skin (login/setup incl. remember-me graft) ✓. **⚠ INSTANCE NOTE (not part of sync):** `config.yaml` carries the 2026-07-19 model rename **`kimi-k3`** (`moonshotai/kimi-k3`) — verified working post-rebuild. **Verified (all green):** `make down && make up` → bootstrap `branch=versioned → upgrade head (0005_run_stop_reason)` (NO new migration) → "Application startup complete", 0 boot errors, NO redis/provisioner, `:2026` 200 → **3b sentinel "schema in sync — no missing columns"** (head `0005`) + **6 models** (deepseek-v4-pro/**kimi-k3**/qwen3.6-plus/qwen3.7-plus/qwen3.7-max/mimo-v2.5-pro) → **consolidation VERIFIED OFF** (`DeerMemConfig.consolidation_enabled=False`; `prompts_dir`/`patterns_dir`=None; mode middleware; injection false; backend_config {}) → **3c Aio bash smoke** sandbox `0ce136c4` / `SYNC20260720_SANDBOX_OK / Linux / python3.10 / uv0.8.9` → **BONUS** `DeerFlowClient.chat(model_name="kimi-k3")` → `KIMI_K3_OK`. Rollback tag `pre-sync-20260720` → `cfca2237` (drop post-push).

Earlier 2026-07-19 (later²) — **8 commits (`90d511f3`→`d2f8f61e`), CLEAN merge (0 conflicts — login/setup UNTOUCHED, no re-skin collision this time), MIGRATION-FREE (head stays `0005`), NO CVE.** Merge `cb2a064d`; `main` ff'd → `d2f8f61e`. Scope: 28 files, +2036/−90. **The 8:** `#4295` `4746a257` fix(loop-detection): clear the per-tool frequency counter on evict/reset — **RELEVANT** (our long qwen research runs have tripped the tool-frequency hard limit); `#4293` `dd2f73c1` fix(config): normalize the `postgres://` short scheme for the async ORM engine (benign robustness); `#4297` `d2f8f61e` fix(skills): add `security_fail_closed` for moderation-model outages (opt-in — `skill_evolution` is OFF in our instance → INERT); `#4262` `90d511f3` + `#4298` `271a921b` refactor(sandbox): E2B client lifecycle helpers/kill-reuse — **INERT** (we run the AIO sandbox, not E2B; the "consolidate" in #4262's title is E2B lifecycle, NOT memory consolidation); `#4288` `bc9c027a` fix(uploads): claim the converted-markdown filename before writing — ⚠ touches `client.py` but ONLY the file-upload→markdown path; **`DeerFlowClient.chat()` interface UNCHANGED → LGI pipeline unaffected** (re-verified post-rebuild); `#4278` `65792c20` fix(frontend): encode artifact URL path segments (inert); `#4025` `86723538` Monocle trace-based behavioral tests (**TEST-ONLY** — new `backend/tests/monocle/`). **Memory subsystem UNTOUCHED — consolidation STILL OFF** (no memory-consolidation commit in range; `DeerMemConfig.consolidation_enabled=False` re-confirmed via `from_backend_config({})`). **`config.example.yaml`** = `config_version 26→27` + new `skill_evolution.security_fail_closed: true` (defaults SAFE/fail-closed; `skill_evolution` disabled in our instance → inert) — ⚠ upstream added **config-version enforcement** (`test_config_version.py`), but the bump caused **NO startup warning/block** against our instance `config.yaml` (bootstrap clean). **Carried patches intact:** Dockerfile readabilipy+Playwright ✓, `.dockerignore` `.deer-flow` ✓, `UV_EXTRAS=postgres` ✓, WRI re-skin ✓ (login/setup incl. the prior-sync remember-me graft, untouched). **⚠ INSTANCE NOTE (not part of this sync):** the gitignored `config.yaml` also carries a same-day model rename **`kimi-k2.6`→`kimi-k3`** (`moonshotai/kimi-k3`, OpenRouter) — verified working post-rebuild. **Verified (all green):** `make down && make up` → bootstrap `branch=versioned → upgrade head (0005_run_stop_reason)` (NO new migration) → "Application startup complete", 0 boot errors, NO redis/provisioner, `:2026` 200 → **3b sentinel "schema in sync — no missing columns"** (head `0005`) + **6 models** (deepseek-v4-pro/**kimi-k3**/qwen3.6-plus/qwen3.7-plus/qwen3.7-max/mimo-v2.5-pro) → **consolidation VERIFIED OFF** (`DeerMemConfig.consolidation_enabled=False`; mode middleware; injection false; backend_config {}) → **3c Aio bash smoke** sandbox `76e55524` / `SYNC20260719C_SANDBOX_OK / Linux / python3.10 / uv0.8.9` → **BONUS** `DeerFlowClient.chat(model_name="kimi-k3")` → `KIMI_K3_OK`. Rollback tag `pre-sync-20260719c` → `ad75ed56` (drop post-push).

Earlier 2026-07-19 (later) — **5 commits (`e89edb39`→`0f088033`), 2 conflicts (login+setup re-skin — kept OURS skin, grafted upstream remember-me), MIGRATION-FREE (head stays `0005`), NO CVE.** Merge `6e13117a`; `main` ff'd → `0f088033`. Scope: 34 files, +1469/−80. **⚠ HEADLINE — `#4255` `a028dfd5` feat(auth): "keep me signed in" — the FIRST login/setup re-skin conflict in weeks.** Upstream adds a `RememberSessionOption` control + a `remember_me` field on every auth submit path (password login/signup, SSO OAuth URL, `/api/v1/auth/initialize` + change-password). **All the LOGIC auto-merged** (rememberMe state, the fetch payloads, `saveRememberLoginPreference`, and the new files `backend/app/gateway/auth/session_cookie.py`, `core/auth/remember-login.ts`, `components/auth/remember-session-option.tsx`); only the JSX conflicted where it overlapped our WRI re-skin. **Resolved keep-OURS-skin + graft-upstream-logic:** kept the maroon `#7b1e2b` buttons, uppercase field labels, the admin-setup callout, and OURS' skinned SSO block; grafted `<RememberSessionOption>` above each submit button (login form + BOTH setup forms) and appended `&remember_me=${rememberMe}` to OURS' SSO OAuth URL; took THEIRS `remember_me` in the initialize payload. **The other 4 (all backend/infra — none touch the embedded `DeerFlowClient` research path used by LGI pipeline + outreach):** `#4284` `e89edb39` fix(gateway): reject non-positive read limits (query validation); `#4281` `75fa028e` fix(artifacts): serve inline binary artifacts via FileResponse for Range support; `#4242` `283cea56` fix(scripts): broaden support-bundle secret-key redaction denylist (**security hardening**); `#4283` `0f088033` fix(gateway): prefer `X-Trace-Id` over `metadata.deerflow_trace_id`. **Memory subsystem UNTOUCHED — consolidation STILL OFF** (no memory commit in range; `DeerMemConfig.consolidation_enabled=False` re-confirmed via `from_backend_config({})` + field default). **`config.example.yaml` UNCHANGED** (not among the 34). **Carried patches intact:** Dockerfile readabilipy+Playwright ✓, `.dockerignore` `.deer-flow` (root, lines 71-72) ✓, `UV_EXTRAS=postgres` ✓, WRI re-skin ✓ — **⚠ UPDATED: login/setup now carry the grafted "keep me signed in" control; future syncs, that IS OURS now.** **Verified (all green):** `make down && make up` → bootstrap `branch=versioned → upgrade head (0005_run_stop_reason)` (NO new migration) → "Application startup complete", 0 boot errors, NO redis/provisioner, `:2026` 200 → **3b sentinel "schema in sync — no missing columns"** (head `0005`) + **6 models** (deepseek-v4-pro/kimi-k2.6/qwen3.6-plus/qwen3.7-plus/qwen3.7-max/mimo-v2.5-pro) → **consolidation VERIFIED OFF** (`DeerMemConfig.consolidation_enabled=False`; mode middleware; injection false; backend_config {}) → **3c Aio bash smoke** `get_sandbox_provider().acquire_async(user_id=sync-smoke-20260719b)` → sandbox `a4f1d670` / `SYNC20260719B_SANDBOX_OK / Linux / python3.10 / uv0.8.9` (no-gem — benign all-in-one-sandbox image drift, as since 2026-07-15). **BONUS — auth pages verified LIVE post-recreate:** `/login` + `/setup` render **200** carrying BOTH the WRI skin (`7b1e2b`, "World Research Institute") AND the grafted **"Keep me signed in"** control. Rollback tag `pre-sync-20260719b` → `53572337` (drop post-push).

Earlier 2026-07-18 (later) — **5 commits (`c9b6131f`→`f8bef42a`), CLEAN merge (0 conflicts), MIGRATION-FREE (head stays `0005`), NO CVE.** Merge `3aac842f`; `main` ff'd → `f8bef42a`. Scope: 13 files, +1052/−33; NO landing/login/setup/i18n/Dockerfile/redis/consolidation touched → no re-skin conflict (merge-tree 0). **⚠ nginx config touched** (`docker/nginx/nginx.conf` + `nginx.local.conf` + helm configmap) but nginx is **NOT a carried patch** (upstream-tracked in local-fixes — only merge commits touch it) → clean merge, and `:2026` verified **200** post-recreate. **The 5:** `#4268` `a0e1d82e` fix(workspace-changes): offload blocking filesystem IO in the text-cache lifecycle (backend blocking-IO robustness); `#4277` `7757e38b` fix(nginx): allow long chat prompts through `/api/langgraph/` without a raw 500 — **LOW relevance** (LGI pipeline + outreach use the EMBEDDED `DeerFlowClient` via docker exec, NOT the HTTP path); `#4250` `5994fdf3` fix: avoid forcing frontend nginx upgrades; `#4280` `f8bef42a` fix(wechat): validate timing config (INERT — no IM channels); `#4270` `208df493` doc(changelog). **Memory subsystem UNTOUCHED — consolidation STILL OFF** (`DeerMemConfig.consolidation_enabled=False` re-confirmed: default + `backend_config: {}` + runtime). **`config.example.yaml`** = COMMENT-ONLY (tidies the commented-out WeChat channel example: `polling_timeout`/`polling_retry_delay`/qrcode comments) → NO key/default change; instance `config.yaml` untouched. **Carried patches intact:** Dockerfile readabilipy+Playwright ✓, `.dockerignore` `.deer-flow` ✓, `UV_EXTRAS=postgres` ✓, WRI re-skin ✓. **Verified (all green):** `make down && make up` → bootstrap (NO new migration, head `0005`) → "Application startup complete" ~3s, 0 boot errors, NO redis/provisioner, `:2026` 200 (nginx serves post-config-change) → **3b sentinel "schema in sync — no missing columns"** (head `0005`) + **6 models** (deepseek-v4-pro/kimi-k2.6/qwen3.6-plus/qwen3.7-plus/qwen3.7-max/mimo-v2.5-pro) → **consolidation VERIFIED OFF** (`DeerMemConfig.consolidation_enabled=False`; mode middleware; injection false; backend_config {}) → **3c Aio bash smoke** `get_sandbox_provider().acquire_async(user_id=sync-smoke-20260718b).get(sid).execute_command(...)` → sandbox `c9d97cb9` / `SYNC20260718B_SANDBOX_OK / Linux / python3.10 / uv0.8.9 / gem`. Rollback tag `pre-sync-20260718b` → `3906071a` (drop post-push).

Earlier 2026-07-18 — **3 commits (`9a4c72db`→`c9b6131f`), CLEAN merge (0 conflicts), MIGRATION-FREE (head stays `0005`), NO CVE.** Merge `9f560434`; `main` ff'd → `c9b6131f`. Scope: 14 files, +1096/−27; NO landing/login/setup/i18n/Dockerfile/redis/consolidation touched → no re-skin conflict (merge-tree 0). **The 3:** `#4246` `5a5c661e` fix(middleware): recover malformed tool-call ids in dangling repair — robustness for strict OpenAI-compat providers, **RELEVANT to the qwen DeerFlowClient research path** (LGI pipeline + outreach); `#4241` `1ae02913` fix(skills): cap archive entry count in `safe_extract_skill_archive` (zip-bomb/entry-count hardening on skill install — defensive, not the research path); `#4264` `c9b6131f` fix(skills): reload mounted skills without restarting Gateway (additive skills feature; harmless here). **Memory subsystem UNTOUCHED — consolidation STILL OFF** (no memory commit in range; `DeerMemConfig.consolidation_enabled` re-confirmed default `False` + `backend_config: {}` + runtime-resolved). **`config.example.yaml`** = COMMENT-ONLY (expands the `skill_scan.enabled` doc comment to spell out deterministic-analyzer vs always-on safe-extraction coverage) → NO key/default change; instance `config.yaml` untouched. **Carried patches intact:** backend/Dockerfile readabilipy+Playwright ✓, `.dockerignore` `.deer-flow` ✓, `UV_EXTRAS=postgres` ✓, WRI re-skin ✓ (all UNTOUCHED — merge-tree 0). **Verified (all green):** `make down && make up` (gateway+frontend rebuilt) → bootstrap (NO new migration, head `0005`) → "Application startup complete" ~6s, 0 boot errors, NO redis/provisioner, `:2026` 200 → **3b sentinel "schema in sync — no missing columns"** (head `0005`) + **6 models** (deepseek-v4-pro/kimi-k2.6/qwen3.6-plus/qwen3.7-plus/qwen3.7-max/mimo-v2.5-pro) → **consolidation VERIFIED OFF** (`DeerMemConfig.consolidation_enabled=False`; mode middleware; injection false; backend_config {}) → **3c Aio bash smoke** `get_sandbox_provider().acquire_async(user_id=sync-smoke-20260718).get(sid).execute_command(...)` → sandbox `3860de06` / `SYNC20260718_SANDBOX_OK / Linux / python3.10 / uv0.8.9 / gem`. Rollback tag `pre-sync-20260718` → `8c77d3df` (drop post-push).

Earlier 2026-07-17 — **13 commits (`b3a0dac8`→`9a4c72db`), CLEAN merge (0 conflicts), MIGRATION-FREE (head stays `0005`), NO CVE.** Merge `dc29032a`; `main` ff'd → `9a4c72db`. Scope: 76 files, +6075/−284; NO landing/login/setup/i18n/Dockerfile/redis/consolidation touched → no re-skin conflict (merge-tree 0). ⚠ **Dating note:** the system clock read **2026-07-17** for this sync although the entry below is labeled 2026-07-19; the sequence is authoritative **by code** — this continues exactly from the prior tip `b3a0dac8` — not by the calendar labels. **⚠ HEADLINE — authz Phase-1A scaffolding (`#4203` `10890e10`):** propagates a trusted authorization Principal context (new `deerflow/authz/principal.py` + `test_authorization_principal.py` + 3 planning docs under `docs/plans/`); **ADDITIVE/INERT** — Phase 1A provides only the trusted identity chain; automatic Layer-1 filtering + Layer-2 enforcement middleware remain separate future work, and no guardrail provider is configured → zero runtime gating. **Memory subsystem: consolidation STILL OFF** — NO memory-consolidation commit in range; the one `<memory>`-tagged commit (`#4209` `e7d7da9e`) is a FRONTEND streamdown fix (strips `<memory>` tags to avoid a React console error), unrelated to the consolidation subsystem. `DeerMemConfig.consolidation_enabled` re-confirmed default `False` in the merged code + our `backend_config: {}` + runtime-resolved `False`. **Security:** `#4253` `ae223199` escape MindIE tool-response content against a `</tool_response>` breakout (prompt-injection hardening — matters for the LGI-outreach untrusted-content path). **New middlewares (additive):** `agents/middlewares/skill_tool_policy_middleware.py` (`#4098` apply allowed-tools only to ACTIVE skills — passive/enabled skills no longer clamp the toolset) + `agents/middlewares/tool_output_synopsis.py` (`#3377` structured typed synopsis for oversized tool-output previews). **Other:** `#4245` preserve single tildes in markdown, `#4219` refuse empty SOUL.md updates in update_agent, `#4235` TUI interrupt an active run before /quit, `#4247` mcp passive-skill-tool-visibility test. **Inert (DeerFlowClient research path uses no IM channels / no provisioner):** `#4229`/`#4251`/`#4231` Feishu/connect-binding/WeChat channel fixes, `#3929`/`#4190` helm sandbox Services default ClusterIP. **`config.example.yaml`** = COMMENT-ONLY (tool_output synopsis doc + an optional `preview_head/tail` sampling note) → NO consolidation/memory-default change; our gitignored `config.yaml` governs. **Carried patches intact:** backend/Dockerfile readabilipy+Playwright ✓, `.dockerignore` `.deer-flow` ✓, `UV_EXTRAS=postgres` ✓, WRI re-skin ✓ (all UNTOUCHED — merge-tree 0). **Verified (hardened, all green):** `make down && make up` (gateway+frontend rebuilt) → bootstrap (NO new migration, head stays `0005`) → "Application startup complete" <3s, 0 boot errors, NO redis/provisioner, `:2026` 200 → **3b sentinel "schema in sync — no missing columns"** (head `0005`) + **6 models** (deepseek-v4-pro/kimi-k2.6/qwen3.6-plus/qwen3.7-plus/qwen3.7-max/mimo-v2.5-pro) → **consolidation VERIFIED OFF** (`DeerMemConfig.consolidation_enabled=False` — default in merged code + `backend_config: {}` + runtime-resolved; mode middleware; injection false) → **3c Aio bash smoke** `get_sandbox_provider().acquire_async(user_id=sync-smoke-20260717).get(sid).execute_command(...)` → sandbox `2ba39c90` / `SYNC20260717_SANDBOX_OK / Linux / python3.10 / uv0.8.9 / gem` (gem canary back). Rollback tag `pre-sync-20260717` → `14157fb4` (drop post-push).

Earlier 2026-07-19 — **7 commits (`45865e9f`→`b3a0dac8`), CLEAN merge (0 conflicts), + NEW MIGRATION `0005_run_stop_reason` (head 0004→0005), NO CVE.** Merge `b1c933a0`; `main` ff'd → `b3a0dac8`. Scope: 43 files, +2091/−375; NO landing/login/setup/i18n/Dockerfile/redis touched → no re-skin conflict (merge-tree 0). **⚠ HEADLINE — first new migration since 0004: `0005_run_stop_reason` (#4188)** = additive/safe (a single NULLABLE `runs.stop_reason` VARCHAR(50) via `safe_add_column`, no backfill/default); applied cleanly on `make up` bootstrap (`branch=versioned → upgrade head 0005`, log "Running upgrade 0004_run_ownership -> 0005_run_stop_reason"), sentinel confirms it. **Memory subsystem touched again (3 commits, consolidation STILL off):** `#4143` LLM-assigned per-fact expiry (new `backend_config` fields `staleness_max_lifetime_multiplier: 20.0` / `staleness_max_extension_days: 3650`; staleness review gains an EXTEND branch — our `backend_config: {}` → all DEFAULT, moot with injection off); `#4217` treat explicit null `backend_config` values as omitted in DeerMemConfig (robustness for a null `model:` key — WE DON'T HIT IT, ours is `{}` with no null model); `#3556` record effective memory identity per run. **Config re-verify (rigorous):** our RESTRUCTURED `config.yaml` (from 2026-07-15: `backend_config: {}` + explicit mode/manager_class/injection_enabled) went LIVE this recreate and resolves clean — `mode=middleware`, `injection_enabled=False`, `manager_class=deermem`, **`DeerMemConfig.consolidation_enabled=False` RESOLVED** (rigorously instantiated under the new #4217/#4143 code; new staleness fields default 20.0/3650). Other: `#4215` subagent checkpoint namespace, `1769b2de` run stop_reason from context. **Inert:** `#4222`/`#4218` channel/GitHub @mention+case fixes (no IM/GitHub channels). **Carried patches intact** (backend/Dockerfile readabilipy+Playwright, `.dockerignore` `.deer-flow`, `UV_EXTRAS=postgres`, WRI re-skin untouched; redis defn present but NO redis container post-`up`). **Verified (hardened, all green):** `make up` (gateway+frontend rebuilt) → bootstrap `upgrade head 0005` (NEW migration applied) → "Application startup complete", 0 boot errors, NO redis/provisioner, `:2026` 200 → **3b sentinel "schema in sync — no missing columns"** (head `0005`, incl. new `stop_reason` col) + **6 models** (deepseek-v4-pro/kimi-k2.6/qwen3.6-plus/qwen3.7-plus/qwen3.7-max/mimo-v2.5-pro) → **consolidation VERIFIED OFF** (`DeerMemConfig.consolidation_enabled=False`; mode middleware; injection false; backend_config {}) → **3c Aio bash smoke** `get_sandbox_provider().acquire_async(user_id=e778f372…).get(sid).execute_command(...)` → sandbox `3c4103ce` / `SYNC0716_SANDBOX_OK / Linux / python3.10 / uv0.8.9`. Rollback tag `pre-sync-20260719` → `cb7325ee` (drop post-push).
...
### 2026-08-16 sync — `main`@`8be08101` (20 commits), merge `a918bd5b` + fixes `e8d54ef5`
- **Merge:** one conflict (compose, redis-strip surface): kept strip, DROPPED upstream's `depends_on: redis: service_healthy`, GRAFTED the gateway healthcheck (nginx's new `service_healthy` dependency requires it). Dockerfile/.dockerignore auto-merged, carried content verified line-by-line. uv 0.7.20→0.11.1, `uv sync --locked` + `uv run --no-sync` (deterministic startup — `make up` now waits on a real `/health` probe). New `.dockerignore` un-ignores for `backend/extensions/sources/` coexist with our excludes.
- **Config:** `make config-upgrade` → v34; NEW sections both land safe: `run_ownership.heartbeat_enabled: false` (standing GATEWAY_WORKERS=1 rule holds), `authorization.enabled: false`. All instance items intact (retrieval_adapter '', sandbox 1.11.0 pin, qwen3.8-max, kimi-k3).
- **Schema:** alembic head → `0012_mcp_task_results` (additive; MCP tasks runtime still OFF). Sentinel: in sync.
- ⚠⚠ **3b PROBE API DIED AGAIN (2nd time):** `deerflow.config.load_config` is GONE → use **`from deerflow.config.app_config import get_app_config`**. Direct `DeerMemConfig(**backend_config)` is NO LONGER REPRESENTATIVE (our config's explicit `model: None` fails validation; the real path resolves a host LLM). **Canonical consolidation probe now:** `DeerMem.from_config(dict(cfg.memory.backend_config or {}), mode="middleware", host_llm_factory=lambda: "stub")._config.consolidation_enabled` (module `deerflow.agents.memory.backends.deermem.deer_mem` — the whole memory package moved under `deerflow.agents.memory`). Verified False + zero memory errors in the boot log.
- ⚠⚠ **BROWSER BROKE TWO WAYS this sync — both mechanisms PROVEN by paired spawn probes, not inferred:**
  1. **Registry-path env never reaches spawned MCPs.** The MCP SDK spawns stdio servers with a MINIMAL default env — the image's `PLAYWRIGHT_BROWSERS_PATH` is invisible to the child (empty-env spawn → `chrome-for-testing is not installed`; env-with-path → LAUNCH-OK). Why it ever worked pre-sync is UNKNOWN (old image gone — do not invent a story). **Fix (durable, Dockerfile `e8d54ef5`):** symlink `/root/.cache/ms-playwright → /ms-playwright` so env-less spawns find the browsers; belt: `PLAYWRIGHT_BROWSERS_PATH` in the server's `env` in extensions_config.json.
  2. **The session pool's TMPDIR pinning breaks chromium via the 108-char unix-socket limit.** `_prepare_stdio_workspace` pins TMPDIR to `<workspace>/.mcp/tmp` (~80+ chars); chromium's singleton socket then exceeds `sun_path` → `Target page, context or browser has been closed` at launch. Proven: `/tmp/m` OK vs `/tmp/xxx…(80 chars)` FAIL — length alone, same filesystem. **Fix (instance-only, extensions_config.json):** playwright server `env` sets `TMPDIR/TMP/TEMP=/tmp` — the pool's `setdefault` respects operator env, so no code patch. ⚠ extensions_config.json is bind-mounted `:ro`; a gateway RESTART picks it up.
  3. 📌 **LESSON (repeat of 08-12, now permanent): the agent-path smoke can print "PLAYWRIGHT OK" while the browser is DEAD** (model falls back to Jina). The ONLY trustworthy browser gate is `smoke_pw3` (PW_STRONG asserts TITLE + WRI + no 'not installed'). Tighten smoke_sub_pw's assertion or ignore its browser half.
- **3c final:** chat×3 (kimi/qwen/deepseek) OK, thread-id 64/65 OK, UAE e2e OK, sandbox bash+grep OK, present_files OK, subagent OK, **PW_STRONG PASS** (WRI login title via real browser), re-skin checklist 3+3 locale refs / login 200 / 0 deerflow.tech / api 401, post-restart chat OK.
- ⚠ NOTE: `extensions_config.json` playwright `env` block is now LOAD-BEARING (browsers path + short TMPDIR) — add to the instance-only rebuild-loss checklist.

### 2026-08-18 sync — `main`@`0debff98` (15 commits), merge `36cd0f70`
- **Merge:** clean, zero conflicts. No migrations, no Dockerfile/compose/config_version changes. i18n locales touched with NEW keys only (`modelLoadFailed`/`modelLoadRetry` for #4840's banner) — tally still 32 en / 31 zh, 3+3 upstream refs. Both playwright load-bearing bits (Dockerfile symlink + extensions_config `env`) verified present.
- **Range highlights:** #4789 hybrid fact-eviction (`fact_eviction_policy`, code default `confidence` = historical — **now a 3b invariant, verified `confidence` by instantiation**), #4804 bootstrap-fact scoping, #4783 Honcho validation (unused), #4667 dynamic-context middleware first-turn targeting (our every-turn path — chat×3 covers), #4827 authenticated frontend reads, #4825/#4830 skills projection copy + fail-closed, MiniMax Code ACP (commented example only), E2B bounds (unused).
- **3b:** sentinel in sync; head `0012_mcp_task_results` (unchanged); consolidation False, eviction `confidence`, retrieval_adapter '', sandbox 1.11.0, plugins [], authorization False, heartbeat False — all via `get_app_config` + `DeerMem.from_config(...)._config`; zero memory errors at boot. ⚠ alembic-head probe: sync `create_engine` fails (no psycopg2 in image) — read via **`asyncpg.connect(os.environ["DATABASE_URL"])`**.
- **3c:** chat×3 OK, thread-id 64/65 OK, UAE e2e OK, sandbox bash+grep OK, present_files OK, subagent OK, **PW_STRONG PASS**, login 200 / 0 deerflow.tech / api 401.

### 2026-08-21 sync — `main`@`a5acc25d` (4 commits), merge `5633d115`
- **#4853 ABSORBED our `.dockerignore` `.deer-flow` carry**: upstream now excludes `**/.deer-flow/` + `**/.env` + `backend/sandbox/` for our exact reason. Conflict resolved keep-THEIRS; remaining carry on that file is ONLY the `docs/` exclude. Carried-patch table: `.dockerignore` row now reads "excludes docs/" only.
...
### 2026-08-25 sync — `main`@`2a261d22` (7 commits), merge `f88dd333`
- **Merge:** CLEAN, zero conflicts; locale changes are new keys only (branched threads #4983, batches UI) — **zero new branded strings**, tallies stay 3+3; all carries intact (backend/Dockerfile symlink :139, .dockerignore docs/, compose redis-strip; only docker-compose-DEV touched upstream, #4956).
- **Schema:** head `0015` → **`0016_subagent_batches`** (auto-applied). Config v34 → **v36**: new `subagent_runtime` (max_running 3, queue admission — shapes subagent traffic only; stage-1c runs subagents OFF) + `subagent_batches` (**`enabled: false` default — NEW 3b invariant, verified False by instantiation**; enabling needs sqlite/postgres backend and materially raises model usage per upstream).
...
- Post-sync app test: Daily Watch brief PASSED vs the rebuilt gateway (valid run, items produced; artefacts discarded — the day’s record stays the 04:35 run).
...
- **3c:** chat×3 OK; **sandbox-real PASS through #5077's modified mount path (container `d136715ed6f7`)**; present_files OK; subagent OK; thread-id 64/65 contract intact; **PW_STRONG PASS** (critical this sync — #5022 expanded the gateway MCP router that writes extensions_config by +649 lines; env survived); re-skin local login 200 / 17 WRI / 0 deerflow.tech; **edge check (new since 30 Aug): Caddy running config still deletes `Authorization` (admin API 127.0.0.1:2019), gateway still hard-401s a raw Basic header (unreachable via the edge).**
- Known benign: same client-side pydantic "Field required" stderr noise on a FileNotFoundError tool-error event as 30 Aug (#4977 watch item — still no gateway-side error, run unaffected).
- Post-sync app test: `lar_watch_brief.py --no-git --no-email` PASSED against the rebuilt gateway (319 s, 5 items, valid JSON — #4839 sanitization did not disturb the client path); test artefacts discarded so the day’s record stays the 04:35 run.

### 2026-08-30 sync — `main`@`8eda71fd` (29 commits), merge `dd668883`
- ⚠⚠ **POST-SYNC BREAKAGE (found by the owner within the hour): the web UI loops `/login?next=/workspace/…` after the Caddy basic-auth prompt.** Mechanism PROVEN: #5041/#4849 made ANY present `Authorization` header take precedence over the session cookie (`auth_middleware.py`: `authorization is not None and not is_auth_disabled()` → `authenticate_pat` → non-Bearer scheme → hard `401 {"detail":"Invalid token"}`, no cookie fallback). The browser re-sends the edge's `Authorization: Basic …` on every request, Caddy forwards it (nginx access log shows user `brandon` on every line), so `/api/features`, `/api/models`, `/api/threads/search` all 401 → the frontend treats it as logged-out → redirect loop. Local repro: `curl -H "Authorization: Basic Zm9vOmJhcg==" http://127.0.0.1:2026/api/features` → `Invalid token` (without the header → `not_authenticated`). **Fix at the edge, not in the app:** `/etc/caddy/Caddyfile` `reverse_proxy` block gets `header_up -Authorization` (the edge consumes the Basic credential; the app's cookie session and Bearer PATs are unaffected — PAT clients cannot reach the public domain without the basic-auth anyway; our pipelines call the gateway inside the container). Then `caddy validate` + `systemctl reload caddy`. APPLIED 2026-08-30 ~13:30 UTC (owner, root shell); running config verified via the admin API (`curl 127.0.0.1:2019/config/` shows the Authorization header delete). **Caddyfile is host config — add it to the rebuild/host-loss checklist** (basic_auth user `brandon`, `flush_interval -1`, `header_up -Authorization`).
- **Merge:** clean, zero conflicts; locales 3+3 (no new branded strings — #5025 outline nav, #5056 markdown artifact window, #5050 model-name truncation carry no "DeerFlow" copy); carried FOWNER patch (`local_backend.py`) untouched by the range. `config.example.yaml` → `config_version: 38` (only a version bump + new `stream_bridge.heartbeat_interval_seconds` default from #5017; instance `config.yaml` unchanged).
- **Image rebuilt** (required: #5087 renames the `tenki-sandbox` dep → `tenki` in `pyproject`/`uv.lock`; #5031 makes `make up/down` call `scripts/deploy.sh` through `$(BASH)` — our wrapper still runs). Rebuild-loss list re-verified: `config.yaml`, `.env`, `extensions_config.json` (md5 `efba0945…` unchanged), playwright symlink (PW_STRONG PASS).
- **3a/3b:** clean boot; :2026 → 200; alembic bootstrap ran `0016 → 0017_personal_access_tokens` (#5041; feature inert — `authorization.enabled: False` verified by instantiation); sentinel in sync; all invariants OK (consolidation off, eviction `confidence`, authz off, run_ownership off, plugins [], subagent_batches off, head `0017`); gateway log clean.
- **3c:** chat×3 OK; **sandbox-real PASS through the NEW acquire path (#5089 `acquire_serialization.py`/`identity.py`) — ran in container `11b5886c2a49`, gateway `eced90a9818f`**; present_files OK; subagent OK; thread-id 64 accepted / 65 rejected; PW_STRONG PASS; re-skin login 200 / 17 WRI / 0 deerflow.tech (the public domain answers 401 to anonymous curl — its basic-auth gate, expected). #5006 authz-recheck-before-sandbox-reuse = no-op with authz off (sandbox reuse exercised by the smoke).
- ⚠ **WATCH:** the smoke's stdout carried a client-side pydantic validation message (`data.content` / `data.file` "Field required", `input_value={'path': '/mnt/user-data/…', 'error': 'FileNotFoundError'}`) while the agent handled a missing-file read before writing `smoke.txt`; the run completed and the gateway log has no error. Likely #4977 (Command-wrapped tool-result normalisation) changed the shape of a tool-error event that `deerflow.client` parses. Harmless so far; if Daily Watch briefs or stage-1 runs start truncating tool errors, this is the first suspect.
- Post-sync: `lar_watch_brief.py --no-git --no-email` (DeerFlowClient chat with thinking + web tools, 330 s, 3 items, valid JSON, live-page check coherent) PASSED against the rebuilt gateway — the #4977 tool-result change did not disturb the application path.

### 2026-08-28 sync — `main`@`0d97fdc7` (15 commits), merge `08527675` + carry `47fbb879`
- **Merge:** clean, zero conflicts, zero new branded strings (3+3), no migrations (head stays `0016`), no compose/Dockerfile/dockerignore changes, config.example adds only a commented RAGFlow example.
- ⚠⚠ **#4986 sandbox hardening BROKE the Aio sandbox — root cause PROVEN by direct `docker run` reproduction, then fixed with a one-line carry:** upstream keeps CHOWN/SETUID/SETGID/DAC_OVERRIDE for the image's startup but the shipped `all-in-one-sandbox:1.11.0` entrypoint also `chmod`s `/run/user/1000` after chowning it → `Operation not permitted`, container exits 1 within a second (docker events: create/start/die(1)/destroy on EVERY acquire); adding **CAP_FOWNER** alone boots it. Seccomp is NOT the issue (upstream keeps `seccomp=unconfined` by default for the Chromium image). No upstream env hatch ADDS capabilities (`DEER_FLOW_SANDBOX_IMAGE_STARTUP_CAPS=0` only drops all) → **carried patch `--cap-add=FOWNER`** in `deerflow/community/aio_sandbox/local_backend.py` (carried-patch table updated; candidate upstream PR). **Verified on a LIVE sandbox container after the fix:** CapDrop=[ALL], CapAdd=[CHOWN DAC_OVERRIDE FOWNER SETGID SETUID], no-new-privileges + seccomp=unconfined, 2 GiB / 2 CPU / 512 pids, and the port bound to **172.17.0.1:8080 only** (`ss` confirms — the unauthenticated exec API is no longer on external interfaces; `host.docker.internal` resolves to 172.17.0.1 inside the gateway via `host-gateway`, so DooD reach is intact). Sandbox smoke back to 15 s.
- ⚠⚠ **SMOKE LESSON (permanent, same class as PW_STRONG):** the sandbox smoke printed OK with `smoke-42` while every sandbox was dying — qwen FABRICATED the expected echo output when the tool failed (`allow_host_bash: false`, so nothing ran anywhere). Only an UNGUESSABLE assertion is trustworthy: `hostname` → a 12-hex id ≠ the gateway's hostname that matches a running `deer-flow-sandbox-*` container (`smoke_chat3.py` now does this). Also: sandbox containers are named `deer-flow-sandbox-*` — a `grep -v deer-flow-` filter hides them; and backgrounding `sg docker` inside a `( … & )` subshell silently runs nothing — use the tool's background mode.
